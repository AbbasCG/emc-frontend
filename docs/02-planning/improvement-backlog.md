...
| 53 | تحويل الـ**5,494 hex خام** في 384 ملفاً إلى tokens (77% منها القيم الثلاث الأساسية ولها tokens دقيقة: `navy`/`customBlue`/`customOrange`) — البوابة `check:brand` تمنع النمو فقط، والقديم يتقلّص يدوياً؛ بعد كل تنظيف: `node scripts/check-raw-hex.mjs --update` | 🟡 | L | الهوية / M7.C | M7 |
| 54 | تغطية `src/hooks/useStudentDashboardData.ts` (524 سطراً — كتلة الفجوة الباقية في `src/hooks/**` بعد M7.A) | 🟡 | M | الاختبارات | M7 |
| 55 | تتبّع خلل مزوّد تغطية v8: الوحدات المستورَدة كسولاً وغير المنفَّذة تعود «0/0» فتسقط من المقام (284 ملفاً/16,642 سطراً) — يُقاس حالياً بممرّتين عبر `npm run coverage:truth`؛ يُراجَع عند ترقية vitest | 🟡 | M | الأدوات / القياس | M7 |
