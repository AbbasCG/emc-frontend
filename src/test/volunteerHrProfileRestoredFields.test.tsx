...
describe('VolunteerHrProfilePage — restored التخصص الجامعي / نبذة عن المتطوع fields', () => {
  ...
  it('the specialization dropdown offers real academic options including أخرى', async () => {
    ...
    const select = screen.getByRole('combobox', { name: 'التخصص الجامعي' })
    await user.click(select)

    expect(await screen.findByText('علوم الحاسوب')).toBeInTheDocument()
    expect(screen.getAllByText('أخرى').length).toBeGreaterThan(0)
  })

  it('the bio textarea enforces the 500-character limit and shows a live counter', async () => {
    const user = userEvent.setup()
    render(<VolunteerHrProfilePage />)
    await waitFor(() => expect(screen.getByText('نبذة عن المتطوع')).toBeInTheDocument())

    const textarea = screen.getByPlaceholderText('نبذة مختصرة عن خبراتك واهتماماتك المهنية')
  ...
  it('submits university_specialization and professional_bio in the payload', async () => {
    ...
    const bioTextarea = screen.getByPlaceholderText('نبذة مختصرة عن خبراتك واهتماماتك المهنية')
