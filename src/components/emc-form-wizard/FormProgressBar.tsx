...
export function FormProgressBar({ percent, label = 'تقدّم النموذج', className, compact }: Props) {
  ...
  return (
    <div className={cn(compact ? 'space-y-2' : 'space-y-2', className)}>
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-[#2691C2]/20 bg-[#2691C2]/[0.07] px-4 py-2.5">
        <span className="text-[12px] font-black text-[#22334A]">{label}</span>
        <span className="text-sm font-black text-[#2691C2] tabular-nums">{p}%</span>
      </div>
