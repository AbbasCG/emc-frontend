...
function SectionBlock({ id, title, children }: { id: string; title: string; children: ReactNode }) {
  return (
    ...
    >
      <h2 className="mb-2.5 flex items-center gap-2 text-sm font-black text-[#22334A]">
    ...
    <SectionBlock id="overview" title="نظرة عامة">
      <div className="space-y-2.5">
        ...
        {learningItems.length > 0 ?
          <div>
            <p className="mb-1 text-[10px] font-black text-[#22334A]/45">ماذا ستتعلم</p>
        ...
        {derived.learningOutcomesBlock ?
          <div>
            <p className="mb-1 text-[10px] font-black text-[#22334A]/45">المخرجات التعليمية</p>
        ...
        {derived.keywordTags.length > 0 ?
          <div className="flex flex-wrap gap-1">
            {derived.keywordTags.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-[#2691C2]/8 px-2 py-0.5 text-[10px] font-black text-[#22334A]"
              >
                {tag}
              </span>
            ))}
        ...
        {derived.targetAudience ?
          <div>
            <p className="mb-1 text-[10px] font-black text-[#22334A]/45">الفئة المستهدفة</p>
        ...
        {reqUniq.length > 0 ?
          <div>
            <p className="mb-1 text-[10px] font-black text-[#22334A]/45">المتطلبات</p>
        ...
        {derived.methodologyLines.length > 0 ?
          <div>
            <p className="mb-1 text-[10px] font-black text-[#22334A]/45">منهجية التدريب</p>
            <ul className="space-y-0.5">
        ...
        {derived.trainingStats.length > 0 ?
          <div className="flex flex-wrap gap-1">
            {derived.trainingStats.map((s) => (
              <span
                key={s.label}
                className="rounded-lg border border-[#22334A]/8 bg-[#f8fafc] px-2 py-0.5 text-[10px] font-black tabular-nums text-[#22334A]"
              >
...
function CurriculumAccordionGroup({ group }: { group: CurriculumGroup }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#22334A]/8 bg-[#f8fafc]">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-2 px-3 py-2 text-right hover:bg-[#2691C2]/5"
      >
        <div>
          <p className="text-[12px] font-black text-[#22334A]">{group.title}</p>
          <p className="text-[10px] font-semibold text-slate-500 tabular-nums">
            {formatPublicText(items.length)} عنصر
          </p>
        </div>
        <ChevronDown className={cn('h-3.5 w-3.5 shrink-0 text-[#2691C2] transition', open && 'rotate-180')} />
      </button>
      {open ?
        <ul className="space-y-0.5 border-t border-[#22334A]/6 px-2.5 py-1.5">
          {items.map((item) => (
...
export function ScheduleSection({ course, derived }: { course: Course; derived: CourseDetailDerived }) {
  ...
  return (
    <SectionBlock id="schedule" title="الجدول">
      <ol className="relative space-y-0 border-r-2 border-[#2691C2]/20 pr-4">
        {rows.map((row, i) => (
          <li key={row.label} className="relative pb-2.5 last:pb-0">
            <span
              className={cn(
                ...
                row.highlight ? 'bg-[#2691C2]' : 'bg-[#EC943C]',
              )}
              aria-hidden
            />
            {i < rows.length - 1 ?
              <span className="absolute -right-[1px] top-4 h-[calc(100%-0.5rem)] w-0 border-r border-dashed border-[#2691C2]/15" aria-hidden />
            : null}
            <p className="text-[9px] font-black text-slate-400">{row.label}</p>
            {row.label === 'رابط الاجتماع' ?
              <a
                href={row.value}
                className="mt-0.5 block truncate text-[12px] font-bold text-[#2691C2]"
                dir="ltr"
                target="_blank"
                rel="noreferrer"
              >
    ...
    <SectionBlock id="instructor" title="المدرب">
      <div className="flex items-start gap-2.5">
        <img
          ...
          alt=""
          loading="lazy"
          className="h-12 w-12 shrink-0 rounded-lg object-cover ring-2 ring-[#2691C2]/15"
        />
        <div className="min-w-0 flex-1 text-right">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1">
            {coursesTaught ?
              <span className="rounded-md bg-[#22334A]/5 px-1.5 py-0.5 text-[9px] font-black tabular-nums text-[#22334A]">
                دورات: {coursesTaught}
              </span>
            : null}
            {studentsTaught ?
              <span className="rounded-md bg-[#2691C2]/10 px-1.5 py-0.5 text-[9px] font-black tabular-nums text-[#2691C2]">
          ...
          {instructor.email ?
            <a
              ...
              className="mt-1.5 inline-flex items-center gap-1 text-[10px] font-semibold text-[#2691C2]"
              dir="ltr"
            >
...
export function ReviewsSection({
  ...
  return (
    <SectionBlock id="reviews" title="التقييمات">
      <div className="mb-2 flex items-center gap-2 rounded-xl bg-[#2691C2]/8 px-3 py-2">
        <Star className="h-4 w-4 fill-[#EC943C] text-[#EC943C]" />
        <span className="text-lg font-black tabular-nums text-[#22334A]">{formatPublicText(averageRating ?? '—')}</span>
      ...
      <div className="space-y-1.5">
        {reviews.slice(0, 4).map((r) => (
          <article key={r.id} className="rounded-xl border border-[#22334A]/6 bg-[#f8fafc] px-2.5 py-2 text-right">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[12px] font-black text-[#22334A]">{r.author}</p>
              <span className="text-[10px] font-black tabular-nums text-[#EC943C]">{formatPublicText(r.rating)} ★</span>
            </div>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#22334A]/8 bg-[#f8fafc]">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-2 px-3 py-2 text-right text-[12px] font-black text-[#22334A] hover:bg-[#2691C2]/5"
      >
        <span>{item.question}</span>
        <ChevronDown className={cn('h-3.5 w-3.5 shrink-0 text-[#2691C2] transition', open && 'rotate-180')} />
      </button>
      {open ?
        <p className="border-t border-[#22334A]/6 px-3 py-2 text-[11px] leading-5 text-slate-600">{item.answer}</p>
      : null}
    </div>
  )
}
