            ...
            >
              {isActive && !reduce && (
                <motion.div
                  ...
                  className="absolute inset-0 rounded-xl bg-[#22334A]"
                  transition={{ duration: 0.22, ease: 'easeOut' }}
                />
              )}
              <span
                className={`relative z-10 inline-flex items-center gap-1 ${
                  isActive ? 'text-white' : isDone ? 'text-[#2691C2]' : 'text-slate-500'
                }`}
              >
          ...
          >
            <header className="shrink-0 border-b border-slate-200 px-5 py-4 sm:px-6">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#2691C2]">دفعة جديدة</p>
                ...
                <motion.div key={section} {...stepMotion}>
                  {section === 'recipient' && (
                    <section className="space-y-3">
                      <div>
                        <h3 className="text-[13px] font-black text-[#22334A]">بيانات المستفيد</h3>
                      ...
                      <label className="block">
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full rounded-xl border border-slate-200 py-2.5 pe-10 ps-3 text-[13px] font-semibold outline-none focus:border-[#2691C2]/50 focus:ring-2 focus:ring-[#2691C2]/15"
                          />
                        </div>
                          ...
                          {studentResults.map((s) => (
                            <li key={s.id}>
                              <button type="button" onClick={() => selectRecipient(s)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-right hover:bg-slate-50">
                                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#2691C2]/10 text-[10px] font-black text-[#2691C2]">{txInitials(s.name)}</span>
                      ...
                      {form.recipient && (
                        <motion.div initial={{ scale: 0.99, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex items-start gap-3 rounded-xl border border-[#2691C2]/25 bg-[#2691C2]/5 px-3 py-3">
                          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#2691C2]/15 text-[11px] font-black text-[#2691C2]">{txInitials(form.recipient.name)}</span>
                          <div className="min-w-0 flex-1">
                  ...
                  {section === 'relation' && (
                    <section className="space-y-3">
                      <div>
                        <h3 className="text-[13px] font-black text-[#22334A]">ربط الدفعة</h3>
                      ...
                      {form.relation && (
                        <motion.div initial={{ scale: 0.99, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-2 rounded-xl border border-[#EC943C]/25 bg-[#EC943C]/8 px-3 py-3">
                          ...
                          <p className="text-[11px] font-bold text-slate-600">
                            نوع العنصر: <span className="text-[#22334A]">{ENTITY_AR[form.relation.type]}</span>
                          </p>
                  ...
                  {section === 'payment' && (
                    <section className="space-y-3">
                      <div>
                        <h3 className="text-[13px] font-black text-[#22334A]">بيانات الدفع</h3>
                      </div>
                      {form.relation_amount != null && (
                  ...
                  {section === 'proof' && (
                    <section className="space-y-3">
                      <div>
                        <h3 className="text-[13px] font-black text-[#22334A]">إثبات الدفع</h3>
                        <p className="text-[11px] text-slate-500">اختياري — PDF، PNG، JPG حتى 5 ميجابايت</p>
                      </div>
                      <label
                        className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 px-4 py-8 transition hover:border-[#2691C2]/40"
                        onDragOver={(e) => e.preventDefault()}
                        onDrop={(e) => { e.preventDefault(); handleProof(e.dataTransfer.files[0] ?? null) }}
                      >
                        <FileUp className="h-7 w-7 text-[#2691C2]" aria-hidden />
                  ...
                  {section === 'review' && (
                    <section className="space-y-4">
                      <div>
                        <h3 className="text-[13px] font-black text-[#22334A]">مراجعة بيانات الدفعة</h3>
            ...
            <footer className="shrink-0 border-t border-slate-200 px-5 py-3 sm:px-6">
              <div className={`flex items-center gap-2 ${section === 'recipient' ? 'justify-end' : 'justify-between'}`}>
                ...
                {section !== 'review' ? (
                  <button type="button" onClick={goNext} className="rounded-xl bg-[#22334A] px-5 py-2 text-[12px] font-black text-white hover:bg-[#1a2940] sm:min-w-[120px]">
                    التالي
                  </button>
                ) : (
                  <button
                    ...
                    disabled={saving}
                    className="inline-flex min-w-[160px] items-center justify-center gap-2 rounded-xl bg-[#EC943C] px-5 py-2 text-[12px] font-black text-white disabled:opacity-60"
                  >
