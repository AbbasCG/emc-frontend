    ...
    <header dir="rtl">
      ...
      >
        <nav aria-label="مسار التنقل" className="mb-3 flex flex-wrap items-center gap-2 text-[12px] font-bold text-[#64748B]">
          ...
          <span className="font-black text-[#0C2A4B]">المدفوعات اليدوية</span>
        </nav>

        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="text-right">
            <div className="flex items-center gap-2">
              <HandCoins className="h-6 w-6 text-[#0077B6]" aria-hidden />
          ...
          <div className="flex flex-wrap items-center justify-end gap-2.5">
            <button
              ...
              className="inline-flex h-11 items-center gap-2 rounded-2xl border border-[#E2E8F0] bg-white px-4 text-[12px] font-black text-[#0C2A4B] transition hover:border-[#0077B6]/35 hover:bg-[#F6F8FB] disabled:opacity-60"
            >
              <RefreshCw className={`h-4 w-4 text-[#0077B6] ${refreshing ? 'animate-spin' : ''}`} aria-hidden />
              {refreshing ? 'جاري التحديث…' : 'تحديث'}
            </button>
            <button
              type="button"
              onClick={onCreate}
              className="inline-flex h-11 items-center gap-2 rounded-2xl bg-[#F28C00] px-5 text-[12px] font-black text-white shadow-sm transition hover:bg-[#d97f28]"
            >
