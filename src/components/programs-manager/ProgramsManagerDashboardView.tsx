...
function Panel({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <section className={`rounded-2xl border border-[#22334A]/6 bg-white p-5 shadow-[0_1px_3px_rgba(34,51,74,0.06),0_8px_24px_-8px_rgba(34,51,74,0.08)] ${className}`}>
      {children}
    </section>
  )
}
...
function PanelHeader({
  ...
  return (
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#2691C2]">{eyebrow}</p>
        <h2 className="mt-1 text-base font-black text-[#22334A]">{title}</h2>
      </div>
      {action}
    </div>
  )
...
function EmptyInsight({ message, hint, href, label }: { message: string; hint: string; href?: string; label?: string }) {
  return (
    <div className="rounded-xl border border-dashed border-[#22334A]/12 bg-[#f8fafc]/80 px-5 py-8 text-center">
      <p className="text-sm font-black text-[#22334A]/75">{message}</p>
      <p className="mx-auto mt-2 max-w-sm text-[12px] font-medium leading-relaxed text-[#22334A]/50">{hint}</p>
      {href && label ?
        <Link to={href} className="mt-4 inline-flex items-center gap-1 rounded-lg bg-[#22334A] px-4 py-2 text-[11px] font-black text-white transition hover:bg-[#1a2838]">
...
function TrendIcon({ direction }: { direction: TrendDirection }) {
  ...
  if (direction === 'flat') return <Minus size={16} className="text-[#22334A]/45" />
  return <Minus size={16} className="text-[#22334A]/30" />
}
...
function PulseCard({ insight }: { insight: TrendInsight }) {
  const bg =
    ...
    : 'from-[#f8fafc] to-white border-[#22334A]/8'
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-[11px] font-black text-[#22334A]/55">{insight.domain}</p>
        <TrendIcon direction={insight.direction} />
      </div>
      <p className="mt-2 text-xl font-black tabular-nums text-[#22334A]">
        {insight.recentValue}
        {insight.changePercent != null && insight.direction !== 'unknown' ?
          <span className={`ms-2 text-[12px] font-bold ${insight.direction === 'up' ? 'text-emerald-600' : insight.direction === 'down' ? 'text-rose-600' : 'text-[#22334A]/45'}`}>
            {insight.changePercent > 0 ? '+' : ''}{insight.changePercent}%
          </span>
        : null}
      </p>
      <p className="mt-2 text-[11px] font-medium leading-relaxed text-[#22334A]/60">{insight.narrative}</p>
    </motion.div>
  )
}
...
function OperationsChart({ analytics }: { analytics: ProgramsManagerAnalytics }) {
  ...
  return (
    <div>
      <p className="mb-4 text-[12px] font-semibold text-[#22334A]/65">{chartHint}</p>
      <div className="h-[240px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={merged} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#22334A10" vertical={false} />
            ...
            />
            <Line type="monotone" dataKey="registrations" stroke="#2691C2" strokeWidth={2.5} dot={{ r: 3 }} name="registrations" />
            <Line type="monotone" dataKey="courses" stroke="#EC943C" strokeWidth={2} dot={{ r: 2 }} strokeDasharray="4 4" name="courses" />
            <Line type="monotone" dataKey="sessions" stroke="#22334A" strokeWidth={2} dot={{ r: 2 }} name="sessions" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
function ActionQueue({ items }: { items: ActionItem[] }) {
  ...
  const priorityStyle = {
    ...
    medium: 'border-r-[#2691C2] bg-[#2691C2]/5',
  }
  ...
  return (
    <ol className="space-y-2">
      {items.map((item, idx) => (
        <li
          key={item.id}
          className={`rounded-xl border border-[#22334A]/6 border-r-[3px] p-3.5 transition hover:shadow-sm ${priorityStyle[item.priority]}`}
        >
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="grid h-5 w-5 place-items-center rounded-md bg-[#22334A] text-[10px] font-black text-white">{idx + 1}</span>
                <span className="rounded-full bg-white px-2 py-0.5 text-[9px] font-black text-[#22334A]/55 ring-1 ring-[#22334A]/10">
                  {priorityLabel(item.priority)}
                </span>
              </div>
              <p className="mt-2 text-[13px] font-black text-[#22334A]">{item.title}</p>
              <p className="mt-1 text-[11px] font-medium leading-relaxed text-[#22334A]/55">{item.reason}</p>
            </div>
          </div>
          <Link
            to={item.href}
            className="mt-3 inline-flex items-center gap-1 rounded-lg bg-[#22334A] px-3 py-1.5 text-[10px] font-black text-white transition hover:bg-[#1a2838]"
          >
...
function AlertRow({ alert }: { alert: ProgramAlert }) {
  ...
  return (
    <div className={`flex gap-3 rounded-xl border px-3.5 py-3 ${isCritical ? 'border-orange-200 bg-orange-50/80' : 'border-amber-200 bg-amber-50/60'}`}>
      ...
      <p className="text-[12px] font-semibold leading-relaxed text-[#22334A]">{alert.message}</p>
    </div>
  )
}
...
function SessionActionRow({ session }: { session: UpcomingSession }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#22334A]/8 px-3.5 py-3">
      <div className="min-w-0">
        <p className="text-[12px] font-black text-[#22334A]">{session.title || session.course_title || 'جلسة'}</p>
        <p className="mt-0.5 text-[10px] text-[#22334A]/50">
    ...
    <div dir="rtl" className="space-y-6 text-right">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.65fr_1fr]">
        <Panel>
          <PanelHeader
            ...
            action={
              <Link to="/dashboard/admin/kpi" className="text-[11px] font-black text-[#2691C2] hover:underline">
                تفاصيل KPI
              </Link>
            }
          />
        ...
        <Panel className="xl:sticky xl:top-4 xl:self-start">
          <PanelHeader
            ...
            action={<Target size={18} className="text-[#2691C2]" />}
          />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel>
          ...
          {bottleneck ?
            <>
              <p className="text-[13px] font-black text-[#22334A]">{bottleneck.narrative}</p>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#22334A]/8">
                <div className="h-full rounded-full bg-[#EC943C]" style={{ width: `${Math.min(bottleneck.sharePercent, 100)}%` }} />
              </div>
              <Link to={bottleneck.href} className="mt-4 inline-flex items-center gap-1 text-[11px] font-black text-[#2691C2] hover:underline">
        ...
        <Panel>
          ...
          {program_alerts.length === 0 ?
            <div className="flex items-center gap-3 rounded-xl bg-[#f8fafc] px-4 py-5">
              <Sparkles size={18} className="text-[#2691C2]" />
              <p className="text-[12px] font-semibold text-[#22334A]/65">لا تنبيهات نشطة — الوضع التشغيلي مستقر.</p>
            </div>
          : (
        ...
        <Panel>
          ...
          <p className="text-[12px] font-semibold leading-relaxed text-[#22334A]/75">{learningHealth.combinedInsight}</p>
          <div className="mt-4 space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-[#22334A]/8 bg-[#f8fafc] px-3 py-2.5">
              <span className="text-[11px] font-black text-[#22334A]/55">الدورات المنتهية</span>
              <span className="text-lg font-black tabular-nums text-[#22334A]">{summary.ended_courses ?? 0}</span>
            </div>
            <div>
              <div className="mb-1 flex justify-between text-[10px] font-black text-[#22334A]/50">
                <span>الحضور</span>
                <span>{summary.attendance_average > 0 ? `${summary.attendance_average}%` : '—'}</span>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-[#22334A]/8">
                <div className="h-full rounded-full bg-[#2691C2]" style={{ width: `${Math.min(summary.attendance_average, 100)}%` }} />
              </div>
              <p className="mt-1 text-[10px] text-[#22334A]/50">{learningHealth.attendanceNarrative}</p>
            </div>
            <div>
              <div className="mb-1 flex justify-between text-[10px] font-black text-[#22334A]/50">
              ...
              <div className="h-1.5 overflow-hidden rounded-full bg-[#22334A]/8">
                <div className="h-full rounded-full bg-[#EC943C]" style={{ width: `${Math.min(summary.completion_average, 100)}%` }} />
              </div>
              <p className="mt-1 text-[10px] text-[#22334A]/50">{learningHealth.completionNarrative}</p>
            </div>
          </div>
        </Panel>
      </div>
      ...
      {sessionsAction.length > 0 && (
        <Panel>
          <PanelHeader
            ...
            action={
              <Link to="/dashboard/admin/lms/sessions" className="text-[11px] font-black text-[#2691C2] hover:underline">
                كل الجلسات
              </Link>
            }
          />
      ...
      {signals.length > 0 && (
        <Panel>
          <PanelHeader eyebrow="ماذا تغيّر مؤخراً؟" title="إشارات النشاط" action={<BarChart3 size={18} className="text-[#22334A]/35" />} />
          <ul className="divide-y divide-[#22334A]/6">
            {signals.map((item: ActivityItem) => (
              <li key={item.id} className="flex flex-wrap items-baseline justify-between gap-2 py-3 first:pt-0 last:pb-0">
                <p className="text-[12px] font-bold text-[#22334A]">
                  {item.action_label}
                  {item.entity_name ? ` — ${item.entity_name}` : ''}
                </p>
                <p className="text-[10px] font-medium text-[#22334A]/45">
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 rounded-xl border border-[#22334A]/6 bg-[#f8fafc]/80 px-4 py-3">
        <span className="text-[10px] font-bold text-[#22334A]/45">إدارة تفصيلية:</span>
        {[
          ...
          <Link key={link.href} to={link.href} className="text-[10px] font-black text-[#2691C2] hover:underline">
            {link.label}
          </Link>
        ))}
      </div>
