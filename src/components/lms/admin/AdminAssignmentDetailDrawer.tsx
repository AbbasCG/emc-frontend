...
function StatTile({ label, value, accent }: { label: string; value: string | number; accent?: string }) {
  return (
    <div className="rounded-xl border border-[#22334A]/8 bg-gradient-to-br from-white to-[#f8fafc] p-3 shadow-sm">
      <p className="text-[10px] font-black uppercase tracking-wide text-[#22334A]/40">{label}</p>
      <p className={`mt-1 emc-display-num text-xl font-black ${accent ?? 'text-[#22334A]'}`}>{value}</p>
    </div>
  )
}
...
function SubmissionDetailPanel({
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-[#2691C2]/20 bg-[#2691C2]/[0.04] p-4"
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        <div>
          <p className="font-bold text-[#22334A]">{row.student_name ?? '—'}</p>
          <p className="text-[11px] font-semibold text-[#22334A]/50">{row.student_email}</p>
        </div>
        <button type="button" onClick={onClose} className="text-[11px] font-bold text-[#2691C2]">
          إغلاق
        </button>
      </div>
      <div className="space-y-2 text-[12px]">
        <div className="flex justify-between gap-2">
          <span className="text-[#22334A]/45">تاريخ التسليم</span>
          <span className="font-bold text-[#22334A]">{fmtDate(row.submitted_at)}</span>
        </div>
        <div className="flex justify-between gap-2">
          <span className="text-[#22334A]/45">الحالة</span>
          <SubmissionStatusBadge status={row.status} />
        </div>
        {row.grade != null ?
          <div className="flex justify-between gap-2">
            <span className="text-[#22334A]/45">الدرجة</span>
            <span className="font-black text-[#22334A]">{fmtNum(row.grade)}</span>
          </div>
        : null}
        <div className="flex flex-wrap gap-2 pt-1">
          {row.has_file ?
            <span className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-0.5 text-[10px] font-black text-[#22334A]/60 ring-1 ring-[#22334A]/10">
        ...
        {row.text_answer ?
          <div className="mt-2 rounded-xl border border-[#22334A]/8 bg-white p-3">
            <p className="mb-1 text-[10px] font-black text-[#22334A]/40">الإجابة النصية</p>
            <p className="whitespace-pre-wrap text-[12px] font-medium leading-relaxed text-[#22334A]">{row.text_answer}</p>
          </div>
        : null}
        {row.feedback ?
...
export default function AdminAssignmentDetailDrawer({
    ...
    <div className="space-y-4">
      ...
      <EntityDetailSection title="بيانات الواجب" icon={<BookOpen className="size-4" />}>
        ...
        {(a?.description || a?.instructions) ?
          <div className="mt-3 space-y-2 rounded-xl border border-[#22334A]/8 bg-white p-3">
            {a?.description ?
              <div>
                <p className="text-[10px] font-black text-[#22334A]/40">الوصف</p>
                <p className="mt-1 text-[13px] font-medium leading-relaxed text-[#22334A]">{a.description}</p>
              </div>
            : null}
            {a?.instructions ?
              <div>
                <p className="text-[10px] font-black text-[#22334A]/40">التعليمات</p>
                <p className="mt-1 text-[13px] font-medium leading-relaxed text-[#22334A]">{a.instructions}</p>
              </div>
            : null}
          </div>
        : null}
        ...
        <div className="space-y-2">
          {filteredSubmissions.map((s) => (
            <motion.div
              key={s.id}
              whileHover={{ y: -1 }}
              className="flex flex-wrap items-center gap-3 rounded-xl border border-[#22334A]/8 bg-white p-3 shadow-sm"
            >
              <div className="min-w-0 flex-1">
                <p className="font-bold text-[#22334A]">{s.student_name ?? '—'}</p>
                <p className="text-[11px] font-semibold text-[#22334A]/45">{s.student_email}</p>
                <p className="mt-0.5 text-[11px] text-[#22334A]/35">{fmtDate(s.submitted_at)}</p>
              </div>
              <SubmissionStatusBadge status={s.status} />
              {s.grade != null ?
                <span className="rounded-full bg-[#22334A]/5 px-2 py-0.5 text-[11px] font-black text-[#22334A]">
                  {fmtNum(s.grade)} نقطة
                </span>
              : null}
              <div className="flex gap-1">
                {s.has_file ? <Paperclip size={14} className="text-[#2691C2]" /> : null}
                {s.has_text ? <FileText size={14} className="text-[#2691C2]" /> : null}
              </div>
              <button
                type="button"
                onClick={() => setReviewRow(s)}
                className="rounded-lg bg-[#2691C2]/10 px-3 py-1.5 text-[11px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/20"
              >
                مراجعة
              </button>
            </motion.div>
      ...
      : (
        <div className="space-y-2">
          {filteredStudents.map((s, idx) => (
            <div
              key={`${s.user_id ?? idx}`}
              className="flex flex-wrap items-center gap-3 rounded-xl border border-[#22334A]/8 bg-white p-3"
            >
              <div className="min-w-0 flex-1">
                <p className="font-bold text-[#22334A]">{s.name ?? '—'}</p>
                <p className="text-[11px] text-[#22334A]/45">{s.email}</p>
              </div>
              <SubmissionStatusBadge status={s.submission_status} />
              {s.progress_percentage != null && s.progress_percentage > 0 ?
                <span className="text-[11px] font-black text-[#2691C2]">{fmtNum(s.progress_percentage)}%</span>
              : null}
              {s.is_late ?
    ...
    <EntityDetailSection title="إعدادات الواجب" icon={<Settings className="size-4" />}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-[#2691C2] px-4 py-2 text-[12px] font-black text-white shadow-sm"
        >
  ...
  return (
    <EntityDetailDrawer
      ...
      footerSlot={
        detail ?
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold text-[#22334A]/45">
            <Calendar size={12} />
    ...
    >
      <div className="mb-4 flex flex-wrap gap-1 rounded-xl border border-[#22334A]/8 bg-[#f8fafc] p-1">
        {TAB_IDS.map((id) => (
          <button
            key={id}
            type="button"
            ...
            className={`rounded-lg px-3 py-2 text-[12px] font-black transition ${
              activeTab === id
                ? 'bg-white text-[#2691C2] shadow-sm'
                : 'text-[#22334A]/50 hover:text-[#22334A]'
            }`}
          >
            {TAB_LABELS[id]}
          </button>
