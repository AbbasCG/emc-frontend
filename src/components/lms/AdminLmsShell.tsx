...
const variantCls: Record<string, { ring: string; iconBg: string; text: string; glow: string }> = {
  brand:   { ring: 'ring-[#2691C2]/15', iconBg: 'bg-[#2691C2]/10', text: 'text-[#2691C2]', glow: 'from-[#2691C2]/12' },
...
function KpiCard({ label, value, icon: Icon, variant = 'brand' }: LmsKpiDef) {
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl border border-white/70 bg-gradient-to-br ${v.glow} to-white p-5 shadow-[0_8px_30px_-12px_rgba(34,51,74,0.18)] ring-1 ${v.ring} backdrop-blur-sm`}
    >
      <div className="flex items-start justify-between gap-3">
        <p className="text-[11px] font-black uppercase tracking-[0.12em] text-[#22334A]/45">{label}</p>
      ...
      <div className="mt-3 emc-display-num text-[2rem] leading-none text-[#22334A]">{value}</div>
    </motion.div>
  )
}
...
function SkeletonKpi() {
  return (
    <div className="rounded-2xl border border-[#22334A]/6 bg-white/80 p-5 shadow-sm">
...
export default function AdminLmsShell({
  ...
  return (
    <motion.div
      ...
      dir="rtl"
    >
      <nav className="flex items-center gap-1.5 text-xs font-semibold text-[#22334A]/40">
        <span>لوحة الإدارة</span>
        <span className="text-[#22334A]/20">/</span>
        <span>نظام التعلم</span>
        <span className="text-[#22334A]/20">/</span>
        <span className="text-[#22334A]/70">{breadcrumb}</span>
      </nav>

      <div className="relative overflow-hidden rounded-3xl border border-white/60 bg-gradient-to-l from-[#22334A] via-[#1a2840] to-[#2691C2]/90 p-6 shadow-[0_20px_50px_-20px_rgba(34,51,74,0.45)] sm:p-8">
