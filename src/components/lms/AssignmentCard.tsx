...
function statusColors(status: StudentAssignment['status'], overdue: boolean): string {
  ...
  return 'bg-[#0077B6]/8 text-[#1a6fa0] border-[#0077B6]/20'
}
      ...
      className={`flex flex-col gap-4 rounded-2xl border bg-white p-5 shadow-sm transition hover:shadow-md ${
        overdue
          ...
          : dueSoon
            ...
            : 'border-[#0C2A4B]/[0.07] hover:border-[#0077B6]/25'
      }`}
    >
        ...
        <div className={`flex items-center gap-2.5 rounded-xl border px-3.5 py-2.5 ${
          overdue
            ...
            : dueSoon
              ...
              : 'border-[#0C2A4B]/[0.07] bg-slate-50/70'
        }`}>
          <Clock className={`h-4 w-4 shrink-0 ${overdue ? 'text-red-500' : dueSoon ? 'text-amber-600' : 'text-[#0C2A4B]/40'}`} />
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-wide text-[#0C2A4B]/50">موعد التسليم النهائي</p>
            <p className={`text-[13px] font-black tabular-nums ${overdue ? 'text-red-700' : dueSoon ? 'text-amber-700' : 'text-[#0C2A4B]'}`}>
              {dueLabel}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {assignment.max_score != null && (
          <span className="inline-flex items-center gap-1.5 rounded-xl border border-[#0C2A4B]/[0.07] bg-slate-50 px-3 py-1.5 text-[12px] font-bold text-[#0C2A4B]/70">
            <Star className="h-3.5 w-3.5 text-[#F28C00]/80" />
      ...
      {canSubmit && onSubmit && (
        <button
          ...
          className={`mt-auto inline-flex w-full items-center justify-center gap-2 rounded-xl py-2.5 text-[12px] font-black text-white shadow-sm transition hover:brightness-105 ${
            needsResub ? 'bg-orange-500' : overdue ? 'bg-red-600' : 'bg-[#F28C00]'
          }`}
        >
