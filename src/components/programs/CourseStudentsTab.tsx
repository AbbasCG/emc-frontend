  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-black text-[#0C2A4B]">طلاب الدورة</p>
          <p className="text-[11px] font-medium text-[#0C2A4B]/50">{formatEnglishCount(total)} مشترك</p>
        </div>
        <button
          type="button"
          onClick={() => onRequestAdd?.()}
          className="inline-flex items-center gap-1.5 rounded-xl bg-[#0C2A4B] px-4 py-2 text-[11px] font-black text-white transition hover:bg-[#0077B6]"
        >
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[#0C2A4B]/10 bg-white py-2.5 pe-3 ps-9 text-[12px] font-semibold outline-none focus:ring-2 focus:ring-[#0077B6]/30"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="rounded-xl border border-[#0C2A4B]/10 bg-white px-3 py-2.5 text-[12px] font-bold"
        >
      ...
      {loading ?
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-14 animate-pulse rounded-xl bg-[#0C2A4B]/6" />
          ))}
        </div>
      : filtered.length === 0 ?
        <div className="rounded-xl border border-dashed border-[#0C2A4B]/15 bg-[#f8fafc] py-12 text-center">
          <UserX size={28} className="mx-auto text-[#0C2A4B]/25" />
          <p className="mt-3 text-sm font-black text-[#0C2A4B]/65">لا يوجد طلاب مسجّلون</p>
          <p className="mt-1 text-[12px] text-[#0C2A4B]/45">استخدم «إضافة طالب» لتسجيل متعلّم في هذه الدورة.</p>
        </div>
      : (
        <div className="overflow-hidden rounded-xl border border-[#0C2A4B]/8">
          {filtered.map((p) => (
            <div
              key={p.registration_id}
              className="flex flex-wrap items-center justify-between gap-3 border-b border-[#0C2A4B]/6 px-4 py-3 last:border-0 hover:bg-[#0077B6]/4"
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <p className="text-[13px] font-black text-[#0C2A4B]">{p.name}</p>
                </div>
                <p className="mt-0.5 text-[11px] font-mono text-[#0C2A4B]/50 dir-ltr">{p.email}</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-black ${statusClass(p.status)}`}>
                  {statusLabel(p.status)}
                </span>
                <span className="text-[11px] font-bold tabular-nums text-[#0C2A4B]/55" dir="ltr">{formatEnglishPercent(p.progress_pct)}</span>
                {p.user_id != null ?
                  <button
                    type="button"
