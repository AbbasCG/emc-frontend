...
function ProgressCard({ label, pct, color }: { label: string; pct: number | null; color: string }) {
  const v = pct ?? 0
  return (
    <div className="rounded-2xl border border-[#0C2A4B]/[0.06] bg-white p-4 shadow-sm">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard icon={Award} label="المستوى النهائي" value={levelBadge?.cefr ?? summary.finalLevelCefr ?? '—'} sub={levelBadge?.arabic} accent="#F28C00" />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-[16px] border border-[#0C2A4B]/[0.06] bg-white p-4 shadow-sm">
            <p className="mb-3 text-[12px] font-black text-deepBlue">تفصيل الأداء</p>
            <div className="grid gap-3 sm:grid-cols-3">
              <ProgressCard label="الاختبار الكتابي" pct={summary.writtenPct} color="#0077B6" />
              <ProgressCard label="المقابلة الشفوية" pct={summary.oralScore} color="#7c3aed" />
              <ProgressCard label="الإجمالي المركّب" pct={summary.overallScore} color="#F28C00" />
            </div>
          </div>
            ...
            <div dir="ltr" className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="72%">
                  ...
                  <Radar name="الأداء" dataKey="value" stroke="#0077B6" fill="#0077B6" fillOpacity={0.28} strokeWidth={2} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-[16px] border border-[#0C2A4B]/[0.06] bg-white p-4 shadow-sm">
            <p className="mb-3 flex items-center gap-2 text-[12px] font-black text-deepBlue">
              <GitCompare className="h-4 w-4 text-[#0077B6]" />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
              ...
              <li key={ev.id} className="relative pb-5 last:pb-0">
                <span
                  className={`absolute -right-[9px] top-1 h-4 w-4 rounded-full ring-4 ring-white ${
                    ev.done ? 'bg-emerald-500' : ev.current ? 'bg-[#F28C00] animate-pulse' : 'bg-slate-200'
                  }`}
                />
        ...
        <div className="rounded-[16px] border border-[#F28C00]/20 bg-gradient-to-l from-[#F28C00]/5 to-white p-4 shadow-sm">
          <p className="mb-3 flex items-center gap-2 text-[12px] font-black text-deepBlue">
            <Lightbulb className="h-4 w-4 text-[#F28C00]" />
            التوصية النهائية
          </p>
          {levelBadge && (
...
function KpiCard({
  ...
  return (
    <div className="rounded-2xl border border-[#0C2A4B]/[0.06] bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
...
function CompareRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between gap-3 border-b border-slate-50 pb-2 last:border-0">
      ...
      <dd className={`font-black text-deepBlue ${highlight ? 'text-[#F28C00]' : ''}`}>{value}</dd>
    </div>
  )
}
