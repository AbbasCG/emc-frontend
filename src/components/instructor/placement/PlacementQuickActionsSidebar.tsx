...
export function PlacementQuickActionsSidebar({ row, courseId, onApproveLevel, canAssess }: Props) {
  ...
  return (
    <aside className="print:hidden lg:sticky lg:top-4 lg:self-start">
      <div className="rounded-[16px] border border-[#22334A]/[0.06] bg-white p-4 shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
        ...
        <div className="space-y-2">
          {actions.map(({ id, label, icon: Icon, href, onClick, primary, disabled }) => {
            const cls = primary
              ? 'bg-[#2691C2] text-white hover:brightness-105 disabled:opacity-45'
              : 'border border-slate-200 bg-white text-deepBlue hover:border-[#2691C2]/30 hover:bg-sky-50/50 disabled:opacity-45'
