          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 sm:px-6 sm:py-5">
            {loading && !detail ? (
              <div className="flex items-center justify-center py-10">
                <div className="h-7 w-7 animate-spin rounded-full border-4 border-[#0077B6]/20 border-t-[#0077B6]" />
              </div>
            ) : (
                    ...
                    {linkEditing ? (
                      <div className="space-y-2">
                        <input
                          ...
                          dir="ltr"
                          className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-[12px] font-bold text-[#0C2A4B] outline-none focus:border-[#0077B6]/40"
                        />
                        <div className="flex gap-2">
                          <button type="button" disabled={linkBusy || !linkValue.trim()}
                            onClick={() => void handleSaveMeetingLink(linkValue.trim())}
                            className="flex-1 rounded-xl bg-[#0C2A4B] py-1.5 text-[11px] font-black text-white transition hover:brightness-110 disabled:opacity-40">
                            حفظ
                          </button>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <a href={b.meeting_link} target="_blank" rel="noreferrer"
                          className="flex items-center gap-1.5 text-[12px] font-bold text-[#0077B6] hover:underline">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button type="button" onClick={() => { setLinkValue(''); setLinkEditing(true) }}
                          className="flex items-center gap-1 rounded-lg border border-[#0077B6]/25 bg-[#0077B6]/[0.06] px-2 py-1 text-[10px] font-black text-[#0077B6] transition hover:bg-[#0077B6]/[0.12]">
                ...
                <Section title="الطالب">
                  ...
                  {b.student && (
                    <a href={`/dashboard/instructor/courses/${slot.course_id}/placement-students`}
                      className="mt-2 inline-flex items-center gap-1.5 text-[12px] font-bold text-[#0077B6] hover:underline">
                ...
                <Section title="التواصل">
                  {!messageOpen ? (
                    <button type="button" onClick={() => setMessageOpen(true)}
                      className="flex w-full items-center justify-center gap-1.5 rounded-2xl border border-[#0077B6]/25 bg-[#0077B6]/[0.06] py-2.5 text-[12px] font-black text-[#0077B6] transition hover:bg-[#0077B6]/[0.12]">
                    ...
                    <div className="space-y-2">
                      <textarea
                        ...
                        rows={3}
                        className="w-full rounded-2xl border border-slate-200 bg-white p-3 text-[12px] font-semibold text-[#0C2A4B] outline-none focus:border-[#0077B6]/40"
                      />
                      <div className="flex gap-2">
                        <button type="button" disabled={busy || !messageBody.trim()} onClick={() => void handleSendMessage()}
                          className="flex-1 rounded-xl bg-[#0C2A4B] py-2 text-[12px] font-black text-white transition hover:brightness-110 disabled:opacity-40">
                          إرسال (إشعار + بريد)
                        </button>
                    ...
                    {!rescheduleOpen ? (
                      ...
                      <div className="space-y-2">
                        <select
                          ...
                          className="h-10 w-full rounded-2xl border border-slate-200 bg-white px-3 text-[12px] font-bold text-deepBlue outline-none focus:border-[#0077B6]/40"
                        >
            ...
            {confirming && (
              ...
              >
                <div className="w-full max-w-xs rounded-2xl bg-white p-5 text-center shadow-xl">
                  <p className="text-[13px] font-black text-[#0C2A4B]">{confirming.label}؟</p>
...
function Row({ icon, label, dir }: { icon: React.ReactNode; label: string; dir?: 'ltr' | 'rtl' }) {
  return (
    <div className="flex items-center gap-2 text-[12px] font-bold text-[#0C2A4B]">
      <span className="text-[#0C2A4B]/40">{icon}</span>
