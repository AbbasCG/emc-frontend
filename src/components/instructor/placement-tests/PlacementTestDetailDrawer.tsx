    ...
    <AnimatePresence>
      {row && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 sm:p-4" dir="rtl">
          ...
          >
            {/* Fixed header */}
            <div className="shrink-0 border-b border-slate-100 bg-gradient-to-l from-[#22334A] to-[#1a2d44] px-5 py-4 text-white sm:px-6 sm:py-5">
              ...
              {TABS.map(({ id, label, icon: Icon }) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 rounded-xl px-1.5 py-2 text-[10px] font-black transition sm:flex-row sm:justify-center sm:gap-1.5 sm:text-[11px] ${
                    tab === id ? 'bg-[#22334A] text-white' : 'text-deepBlue/55 hover:bg-slate-50'
                  }`}
                >
...
function HeaderBadge({
  ...
  const toneCls = value
    ? tone === 'final' ? 'bg-[#EC943C]/25 text-[#FFD9AE]'
      ...
      {overall != null && (
        <div>
          ...
          <div className="h-2 overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-[#2691C2]" style={{ width: `${overall}%` }} />
          </div>
        </div>
      )}
    </div>
...
function OralTab({ row }: { row: InstructorPlacementTestRow }) {
  ...
  return (
    <div className="space-y-4 p-4 sm:p-6">
        ...
        hasRubric ? (
          ...
          <div className="space-y-2 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-[11px] font-black text-[#2691C2] shadow-sm ring-1 ring-slate-200 transition hover:bg-[#2691C2]/5"
            >
      ...
      <Link
        to={`/dashboard/instructor/courses/${row.course_id}/placement-students`}
        className="flex items-center justify-center gap-2 rounded-xl bg-[#2691C2] px-4 py-2.5 text-[12px] font-black text-white"
      >
  ...
  return (
    <div className="space-y-4 p-4 sm:p-6">
      ...
      <Link
        ...
        className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#2691C2]/30 bg-sky-50 px-4 py-3 text-[12px] font-black text-[#2691C2]"
      >
...
function StatBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-[#F8FAFC] px-3 py-2.5 ring-1 ring-[#22334A]/[0.04]">
