  ...
  return (
    <motion.article
      ...
      className={`flex w-full max-w-[430px] flex-col rounded-[16px] border bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)] transition-all ${
        selected
          ? 'border-[#0077B6]/40 ring-2 ring-[#0077B6]/15 shadow-[0_8px_28px_-12px_rgba(0,119,182,0.28)]'
    ...
    >
      ...
      <button
        type="button"
        onClick={onSelect}
        className="w-full cursor-pointer rounded-t-[16px] p-4 text-right outline-none focus-visible:ring-2 focus-visible:ring-[#0077B6]/50"
      >
        ...
        <div className="flex items-start gap-3">
          {row.avatar_url ? (
            ...
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#0C2A4B]/8 text-[14px] font-black text-deepBlue">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-3 text-[10.5px] font-semibold text-deepBlue/55">
            {submittedDate && (
              <span className="flex items-center gap-1">
                <CalendarDays className="h-3.5 w-3.5 shrink-0 text-[#0077B6]/70" />
                {submittedDate}
              </span>
            )}
            {oralTimeRange && (
              <span className="flex items-center gap-1">
                <Clock className="h-3.5 w-3.5 shrink-0 text-[#0077B6]/70" />
                {oralTimeRange}
              </span>
            )}
          </div>
        ...
        <div className="mt-3">
          ...
          <div className="h-1.5 overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-gradient-to-l from-[#0077B6] to-[#0C2A4B] transition-all" style={{ width: `${Math.min(100, progressVal)}%` }} />
          </div>
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1">
          ...
          {row.is_assigned && (
            <span className="rounded-md bg-[#0077B6]/10 px-2 py-0.5 text-[9px] font-black text-[#0077B6] ring-1 ring-[#0077B6]/20">مُسند</span>
          )}
          {row.oral_booking_at && (
      ...
      <div className="flex flex-wrap gap-1 border-t border-slate-100 p-2">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-[9px] font-black text-deepBlue/55 transition hover:bg-slate-50 hover:text-[#0077B6]"
        >
...
function ScoreMini({ label, value, max, pct }: { label: string; value: number | null; max: number; pct: number | null }) {
  return (
    <div className="rounded-lg bg-[#F8FAFC] px-2.5 py-2 ring-1 ring-[#0C2A4B]/[0.04]">
      ...
      {pct != null && <p className="text-[9px] font-bold text-[#0077B6]">{pct}%</p>}
    </div>
  )
}
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-[9px] font-black transition ${
        primary
          ? 'bg-[#0C2A4B] text-white hover:brightness-110'
          : 'text-deepBlue/55 hover:bg-slate-50 hover:text-[#0077B6]'
      }`}
    >
