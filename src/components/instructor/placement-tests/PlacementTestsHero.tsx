...
export function PlacementTestsHero({ stats, loading, onRefresh, refreshing }: Props) {
  return (
    <div className="relative overflow-hidden rounded-[20px] border border-[#0C2A4B]/[0.06] bg-gradient-to-l from-[#0C2A4B] via-[#1a2d44] to-[#152536] p-5 shadow-[0_16px_48px_-20px_rgba(15,23,42,0.45)] sm:p-6">
      <div className="pointer-events-none absolute -left-10 top-0 h-40 w-40 rounded-full bg-[#F28C00]/15 blur-[60px]" />
      <div className="pointer-events-none absolute -bottom-8 right-0 h-32 w-32 rounded-full bg-[#0077B6]/20 blur-[50px]" />
      ...
      <div className="relative flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
        ...
        >
          <KpiCard icon={Users}         label="إجمالي الطلاب"           value={stats.total}          loading={loading} index={0} tone="text-[#0077B6] bg-[#0077B6]/10" />
          ...
          <KpiCard icon={BarChart3}      label="متوسط الكتابي"           value={stats.avgWritten != null ? `${stats.avgWritten}%` : '—'} loading={loading} index={4} tone="text-[#F28C00] bg-[#F28C00]/15" />
          <KpiCard icon={BarChart3}      label="متوسط الشفوي"            value={stats.avgOral != null ? `${stats.avgOral}%` : '—'}     loading={loading} index={5} tone="text-[#0077B6] bg-[#0077B6]/10" />
        </motion.div>
      </div>
    </div>
  )
