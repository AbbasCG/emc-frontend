    ...
    return (
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex flex-1 items-center justify-center py-20">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#2691C2]/20 border-t-[#2691C2]" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="grid shrink-0 grid-cols-3 gap-2 border-b border-slate-100 bg-slate-50/70 px-4 py-2.5 sm:grid-cols-6 sm:px-6">
        <StatChip label="أسئلة" value={stats.total} color="text-[#22334A]" />
        ...
        <StatChip label="الدرجة" value={subject.score ?? '—'} color="text-[#2691C2]" />
        <StatChip label="النسبة" value={pct != null ? `${pct}%` : '—'} color="text-[#EC943C]" />
      </div>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-slate-100 px-4 py-2.5 sm:px-6">
        {([
          ...
          <button
            ...
            className={`rounded-full px-3 py-1.5 text-[11px] font-black transition ${
              filter === key ? 'bg-[#22334A] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
          >
            {label}
          </button>
        ...
        <div className="relative mr-auto w-full max-w-[220px] sm:w-[220px]">
          <Search className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#22334A]/35" />
          <input
            type="text"
            value={search}
            ...
            dir="rtl"
            className="h-8 w-full rounded-full border border-slate-200 bg-white pe-3 ps-8 text-[11px] font-semibold text-[#22334A] outline-none focus:border-[#2691C2]/40"
          />
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-[10px] font-black tabular-nums transition ${
                dimmed ? 'opacity-25' : ''
              } ${isActive ? 'ring-2 ring-offset-1 ring-[#22334A]' : ''} ${
      ...
      <div className="flex shrink-0 items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 sm:px-6">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-xl bg-[#22334A] px-4 py-2 text-[12px] font-black text-white transition hover:brightness-110 disabled:opacity-35"
        >
          التالي
