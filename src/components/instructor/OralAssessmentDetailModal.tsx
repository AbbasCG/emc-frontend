          ...
          >
              ...
              <div className="space-y-4">
                ...
                <Section title="النتيجة الإجمالية">
                  <div className="flex items-center gap-4">
                    <div className="rounded-2xl border border-[#2691C2]/25 bg-[#2691C2]/[0.06] px-4 py-3 text-center">
                      <p className="text-[9px] font-black uppercase tracking-widest text-[#2691C2]">درجة المقابلة</p>
                      <p className="mt-0.5 font-mono text-[22px] font-black tabular-nums text-[#22334A]">
                        {assessment.oral_score ?? '—'}<span className="text-[12px] text-[#22334A]/40">/{assessment.oral_score_max}</span>
                      </p>
                    </div>
                    {finalCefr && (
                      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-center">
                        <p className="text-[9px] font-black uppercase tracking-widest text-emerald-600">المستوى النهائي</p>
                        <p className="mt-0.5 text-[16px] font-black text-[#22334A]">{finalCefr.cefr}</p>
                        <p className="text-[10px] font-semibold text-[#22334A]/50">{finalCefr.arabic}</p>
                      </div>
                    )}
                ...
                <Section title="معايير التقييم">
                  <div className="space-y-2.5">
                    {assessment.rubric.map((c) => {
                      ...
                      return (
                        <div key={c.key}>
                          <div className="mb-1 flex items-center justify-between text-[11px] font-bold text-[#22334A]">
                            <span>{c.label}</span>
                            <span className="font-mono tabular-nums text-[#22334A]/60">{c.score ?? '—'}/{c.max}</span>
                          </div>
                          <div className="h-1.5 overflow-hidden rounded-full bg-slate-100">
                            <motion.div
                              className="h-full rounded-full bg-[#2691C2]"
                              initial={{ width: 0 }}
                              animate={{ width: `${pct}%` }}
            ...
            <div className="flex shrink-0 items-center justify-between gap-3 border-t border-slate-100 px-5 py-3 sm:px-6">
              <button type="button" onClick={handleEdit}
                className="flex items-center gap-1.5 rounded-xl bg-[#22334A] px-4 py-2 text-[12px] font-black text-white transition hover:brightness-110">
...
function InfoTile({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-white p-2.5 ring-1 ring-black/5">
      ...
      <p className="mt-1 truncate text-[11px] font-black text-[#22334A]">{value}</p>
    </div>
  )
}
...
function NoteBlock({ label, text, tone }: { label: string; text: string; tone: 'emerald' | 'amber' | 'sky' | 'slate' }) {
  ...
  return (
    <div className={`rounded-xl border p-3 ${cls}`}>
      ...
      <p className="text-[12px] font-semibold leading-relaxed text-[#22334A]/85">{text}</p>
    </div>
  )
}
