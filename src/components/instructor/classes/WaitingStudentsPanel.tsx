...
function WaitingStudentsPanelInner({
  ...
  return (
    <aside className="flex flex-col overflow-hidden rounded-[16px] border border-[#22334A]/[0.06] bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
      <div className="border-b border-[#22334A]/[0.05] bg-[#F8FAFC] px-4 py-4">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#EC943C]/15 text-[#EC943C]">
              <Users className="h-4 w-4" />
            </span>
            <div>
              <h2 className="text-[13px] font-bold text-[#22334A]">طلاب بانتظار التعيين</h2>
              <p className="text-[10px] font-medium text-slate-400">توزيع حسب المستوى والسعة</p>
            </div>
          </div>
          <span className="rounded-full bg-[#EC943C]/15 px-2.5 py-0.5 text-[11px] font-bold text-[#EC943C]">
            {unassigned.length}
          </span>
        </div>
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            dir="rtl"
            className="h-9 w-full rounded-xl border border-[#22334A]/10 bg-white pr-8 pl-3 text-[11px] font-semibold text-[#22334A] outline-none focus:border-[#2691C2] focus:ring-2 focus:ring-[#2691C2]/15"
          />
        </div>
      </div>
      ...
      {assigned.length > 0 && (
        <div className="border-t border-[#22334A]/[0.05] bg-[#F8FAFC]/80 px-3 py-2">
          <button
            type="button"
            onClick={() => setShowAssigned((v) => !v)}
            className="w-full rounded-lg py-2 text-[10px] font-bold text-[#22334A]/50 transition hover:bg-white hover:text-[#22334A]"
          >
