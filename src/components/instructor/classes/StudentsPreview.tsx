...
function StudentsPreviewInner({ students, max = 4, size = 'sm' }: Props) {
  ...
  return (
    <div className="flex items-center">
      <div className="flex -space-x-2 space-x-reverse">
        {shown.map((s, i) => (
          <div
            ...
            className={`${dim} relative flex shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-white bg-gradient-to-bl from-[#22334A] to-[#2691C2] font-bold text-white ring-1 ring-[#22334A]/10`}
      ...
      {extra > 0 && (
        <span className="mr-2 text-[10px] font-bold text-[#22334A]/45">+{extra}</span>
      )}
    </div>
  )
}
