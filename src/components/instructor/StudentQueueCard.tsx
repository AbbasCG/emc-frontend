    ...
    <motion.div
      ...
      className={`relative overflow-hidden rounded-[16px] border p-3.5 transition-all duration-200 ${
        s.is_assigned
          ...
          : priority
            ? 'border-[#EC943C]/40 bg-white shadow-[0_4px_16px_-8px_rgba(236,148,60,0.35)]'
            : 'border-[#22334A]/[0.06] bg-white hover:border-[#2691C2]/25 hover:shadow-sm'
      }`}
    >
      {priority && !s.is_assigned && (
        <span className="absolute right-0 top-3 bottom-3 w-0.5 rounded-full bg-[#EC943C]" aria-hidden />
      )}

      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-bl from-[#22334A] to-[#2691C2] text-[13px] font-bold text-white shadow-sm">
          {s.avatar_url ? (
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          ...
          {waitBadge && !s.is_assigned && (
            <span className="flex items-center gap-0.5 rounded-lg bg-[#EC943C]/10 px-2 py-0.5 text-[9px] font-bold text-[#EC943C]">
      ...
      {!s.is_assigned && groups.length > 0 && (
        <button
          ...
          className="mt-2.5 flex w-full items-center justify-center gap-1.5 rounded-xl border border-[#2691C2]/20 bg-[#2691C2]/[0.06] py-2 text-[10px] font-bold text-[#2691C2] transition hover:bg-[#2691C2]/10"
        >
      ...
      <AnimatePresence>
        {showAssign && (
          ...
          >
            <div className="mt-2 space-y-1.5 border-t border-[#22334A]/[0.05] pt-2">
              ...
              {(recommended.length > 0 ? recommended : groups).map((g) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-xl border border-[#22334A]/10 bg-[#F8FAFC] px-3 py-2 text-[10px] font-semibold text-[#22334A] transition hover:border-[#2691C2]/30 hover:bg-white disabled:opacity-50"
                >
