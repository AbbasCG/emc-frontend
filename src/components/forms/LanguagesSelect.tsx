...
import { COMMON_LANGUAGES } from './languagesData'
...
const rtlStyles: StylesConfig<LanguageOption, true> = {
  control: (base, state) => ({
    ...
    borderColor: state.isFocused ? '#0077B6' : '#e2e8f0',
    backgroundColor: state.isFocused ? '#fff' : '#f8fafc',
    boxShadow: state.isFocused ? '0 0 0 4px rgba(0,119,182, 0.12)' : 'none',
    textAlign: 'right' as const,
    direction: 'rtl' as const,
    cursor: 'text',
    '&:hover': { borderColor: state.isFocused ? '#0077B6' : '#cbd5e1' },
  }),
  menu: (base) => ({
    ...base,
    borderRadius: '0.75rem',
  ...
  option: (base, state) => ({
    ...base,
    textAlign: 'right' as const,
    backgroundColor: state.isSelected ? '#0077B6' : state.isFocused ? '#e0f2fe' : '#fff',
