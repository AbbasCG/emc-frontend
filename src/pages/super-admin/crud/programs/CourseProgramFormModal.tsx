...
function ModernTimePicker({ label, value, onChange, error }: TimePickerProps) {
  ...
  return (
    <div ref={containerRef} className="relative text-[11px] font-black text-[#22334A]">
      <span className="mb-1 block">{label}</span>
      <button
        type="button"
        ...
        <div className="absolute inset-x-0 top-full z-[9999] mt-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_60px_-15px_rgba(15,23,42,0.28)]">
          ...
          <div className="flex items-center gap-2 border-b border-slate-100 px-3 py-2">
            <input
              ...
              dir="ltr"
              placeholder="HH:MM"
              className="w-full rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 font-mono text-[13px] font-black text-[#22334A] outline-none focus:border-[#2691C2]/50 focus:ring-2 focus:ring-[#2691C2]/20"
            />
          </div>
          ...
          <div className="flex items-center justify-between border-t border-slate-100 px-3 py-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={apply}
                className="rounded-lg bg-[#2691C2] px-4 py-1.5 text-[12px] font-black text-white transition hover:bg-[#1a7aab]"
              >
                تطبيق
              </button>
            </div>
    ...
    if (currentStep === 4) {
      return (
        <FormSectionCard title="السعر والتسجيل والإدارة الداخلية" eyebrow="الخطوة 4" icon={CreditCard}>
          ...
          <div className="rounded-2xl border border-[#2691C2]/15 bg-[#2691C2]/[0.04] p-4">
            <p className="text-[12px] font-black text-[#22334A]">قيود التسجيل</p>
            <label className="mt-3 flex items-center gap-3 text-[13px] font-black text-[#22334A]">
              <input
                type="checkbox"
            ...
            {requiresRegistrationCode ?
              <label className="mt-3 block text-[11px] font-black text-[#22334A]">
                رمز التسجيل
                <input
      ...
      <>
        {successOpen ?
          <FormSuccessState
            ...
            actions={
              <button type="button" onClick={() => { setSuccessOpen(false); onSaved() }} className="rounded-xl bg-[#22334A] px-4 py-2 text-sm font-black text-white">
                العودة للتفاصيل
              </button>
            }
          />
          ...
          <div className="flex min-h-[65vh] flex-col">
            <div className="shrink-0 space-y-3 border-b border-slate-100 pb-4">
              ...
              <div className="h-1 overflow-hidden rounded-full bg-slate-100">
                <div className="h-full rounded-full bg-gradient-to-l from-[#2691C2] to-[#EC943C]" style={{ width: `${progressPercent}%` }} />
              </div>
              {validationSummary}
            </div>
