...
export default function AdminPermissionConfigTree() {
  ...
  const fetchRoles = async () => {
    try {
      const response = await apiClient.get('/api/admin/permissions/roles');
  ...
  const fetchTree = async (role: Role) => {
    ...
    try {
      const response = await apiClient.get(`/api/admin/permissions/roles/${role.name}/delegatable`);
      if (response.data.success) {
  ...
  const toggleDelegatable = async (node: PermissionNode) => {
    ...
    try {
      await apiClient.post(`/api/admin/permissions/roles/${selectedRole.name}/delegatable/toggle`, {
