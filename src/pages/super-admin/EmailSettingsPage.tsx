...
const CATEGORY_META: Record<string, { label: string; accent: string }> = {
  ...
  support:          { label: 'الدعم الفني',      accent: '#2691C2' },
  courses:          { label: 'الدورات',          accent: '#0EA5E9' },
  certificates:     { label: 'الشهادات',         accent: '#EC943C' },
  users:            { label: 'المستخدمون',       accent: '#22334A' },
  ...
  return (
    <button
      ...
      className={[
        ...
        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#2691C2]',
        disabled || busy ? 'cursor-not-allowed opacity-50' : 'cursor-pointer',
        enabled ? 'bg-[#2691C2]' : 'bg-slate-300',
      ].join(' ')}
    >
      <span
        className={[
      ...
      >
        {busy && (
          <span className="absolute inset-0 flex items-center justify-center">
            <span className="h-3 w-3 animate-spin rounded-full border-2 border-slate-300 border-t-[#2691C2]" />
          </span>
        )}
      </span>
    </button>
    ...
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className={`text-[13px] font-bold ${enabled ? 'text-[#22334A]' : 'text-slate-400'}`}>{item.label}</p>
          {dirty && (
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-[14px] font-black text-[#22334A]">{meta.label}</span>
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] p-6 pb-28" dir="rtl">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#22334A]">
            <Mail className="h-5 w-5 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-black text-[#22334A]">مركز أتمتة البريد الإلكتروني</h1>
        ...
        <button
          ...
          disabled={testing}
          className="flex items-center gap-1.5 rounded-lg border border-[#2691C2] px-4 py-2 text-sm font-bold text-[#2691C2] transition hover:bg-[#EFF6FF] disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-red-100 bg-red-50 py-16 text-center">
          <p className="font-black text-[#22334A]">تعذّر تحميل الإعدادات</p>
          <button
            type="button"
            onClick={() => void load()}
            className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#22334A] px-5 py-2.5 text-[12px] font-black text-white transition hover:brightness-110"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-sm font-black text-[#22334A]">معلومات المُرسِل</h2>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm text-[#22334A] outline-none transition focus:ring-2 focus:ring-[#2691C2]/40 disabled:opacity-60"
                />
              </label>
              <label className="block">
          ...
          <div className="flex w-full max-w-xl items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-3 shadow-2xl shadow-slate-900/10">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={saving}
                className="flex items-center gap-1.5 rounded-xl bg-[#22334A] px-5 py-2 text-[12px] font-black text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {saving ? (
                  <>
