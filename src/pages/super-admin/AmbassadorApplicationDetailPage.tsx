...
function nameToColor(name: string): string {
  const colors = [
    ...
    'from-[#2691C2] to-[#1a7aad]',
    'from-[#EC943C] to-[#d4782e]',
...
function SectionCard({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 border-b border-[#22334A]/[0.05] bg-[#F8FAFC] px-4 py-2.5">
        <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-[#22334A]/[0.06] text-[#22334A]/70">
          {icon}
        </span>
        <h3 className="text-[15px] font-bold tracking-tight text-[#22334A] sm:text-[18px]">{title}</h3>
      </div>
    ...
    <div>
      ...
      {isLong && (
        <button
          ...
          className="mt-1.5 text-[11px] font-bold text-[#2691C2] transition hover:underline"
        >
...
function ChipList({
  ...
  const styles = {
    slate:   'bg-[#22334A]/[0.05] text-[#22334A]',
    blue:    'bg-[#2691C2]/[0.08] text-[#22334A]',
...
function ActionBtn({
  ...
  const variantStyles = {
    neutral: 'border-[#22334A]/10 bg-white text-[#22334A] hover:border-[#22334A]/20 hover:bg-[#F8FAFC]',
    primary: 'border-[#22334A] bg-[#22334A] text-white hover:bg-[#2691C2] hover:border-[#2691C2]',
    danger:  'border-red-200 bg-red-50 text-red-700 hover:bg-red-100',
    accent:  'border-[#2691C2]/25 bg-[#2691C2]/[0.06] text-[#22334A] hover:bg-[#2691C2]/10',
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl" dir="rtl">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={onConfirm}
            className={`flex-1 rounded-xl py-2.5 text-xs font-bold text-white ${danger ? 'bg-red-600 hover:bg-red-700' : 'bg-[#22334A] hover:bg-[#2691C2]'}`}
          >
            {confirmLabel}
          </button>
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 rounded-xl border border-slate-200 py-2.5 text-xs font-bold text-[#22334A]"
          >
            إلغاء
          </button>
        </div>
...
function buildTimeline(app: AmbassadorApplication): TimelineItem[] {
  ...
  if (app.created_at ?? app.submitted_at) {
    ...
    items.push({
      id: 'submitted',
      icon: <Star className="h-3.5 w-3.5 text-[#2691C2]" />,
      title: 'تقديم الطلب',
  ...
  for (const h of (app.status_history as AmbassadorStatusHistory[] | undefined) ?? []) {
    ...
    items.push({
      id: `status-${h.id}`,
      icon: <History className="h-3.5 w-3.5 text-[#22334A]/60" />,
  ...
  for (const n of (app.notes as AmbassadorNote[] | undefined) ?? []) {
    items.push({
      id: `note-${n.id}`,
      icon: <MessageSquare className="h-3.5 w-3.5 text-[#EC943C]" />,
      title: 'ملاحظة داخلية',
    ...
    <ol className="space-y-0">
      {items.map((item, idx) => (
        <li key={item.id} className="relative flex gap-3 pb-5 last:pb-0">
          ...
          <div className="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#F8FAFC] ring-1 ring-[#22334A]/[0.08]">
            {item.icon}
          </div>
  ...
  return (
    <div className="space-y-2.5">
      {notes.map((note) => (
        <div
          key={note.id}
          className="rounded-xl border border-[#22334A]/[0.06] bg-[#F8FAFC] p-4"
        >
          <div className="mb-2 flex items-start justify-between gap-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-semibold text-[#22334A]">{note.author}</span>
              {note.created_at && (
...
export default function AmbassadorApplicationDetailPage() {
  ...
  const avatarGradient = app ? nameToColor(app.full_name) : 'from-[#22334A] to-[#2691C2]'
  ...
  if (loadError ?? !app) {
    return (
      <div className="flex flex-col items-center justify-center rounded-3xl border border-rose-100 bg-white py-24 text-center" dir="rtl">
        ...
        <p className="font-bold text-[#22334A]">{loadError ?? 'الطلب غير موجود'}</p>
        <div className="mt-6 flex gap-2">
          <button
            type="button"
            onClick={() => void loadApp()}
            className="inline-flex items-center gap-2 rounded-xl bg-[#22334A] px-5 py-2.5 text-[12px] font-bold text-white"
          >
          ...
          <button
            type="button"
            onClick={goBack}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-5 py-2.5 text-[12px] font-bold text-[#22334A]"
          >
  ...
  return (
    <div className="pb-16 font-[Cairo,sans-serif]" dir="rtl">
      ...
      {/* ── Sticky page header ────────────────────────────────────────── */}
      <div className="sticky top-0 z-30 mb-5 -mx-4 border-b border-[#22334A]/[0.06] bg-white/90 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:px-6">
        <div className="flex flex-wrap items-center gap-3">
          <button
            type="button"
            onClick={goBack}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-[#22334A]/10 text-slate-500 transition hover:border-[#22334A]/20 hover:text-[#22334A]"
            aria-label="رجوع"
          >
            <ArrowRight className="h-4 w-4" />
          </button>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            >
              {downloadingAll ? (
                ...
                <Download className="h-3.5 w-3.5 text-[#2691C2]" />
              )}
              تحميل الملفات
            </ActionBtn>
            ...
            <button
              ...
              onClick={() => void loadApp()}
              disabled={loading}
              className="flex h-7 w-7 items-center justify-center rounded-xl border border-[#22334A]/10 text-slate-400 transition hover:border-[#22334A]/20 hover:text-[#22334A] disabled:opacity-40"
              aria-label="تحديث"
            >
      ...
      {/* ── Hero card ────────────────────────────────────────────────── */}
      <div className="relative mb-6 overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-[#22334A] via-[#1a2a3a] to-[#2691C2] p-6 text-white shadow-xl sm:p-8">
        <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-white/[0.04]" />
        <div className="pointer-events-none absolute -bottom-12 right-24 h-48 w-48 rounded-full bg-[#EC943C]/[0.10]" />
        <div className="pointer-events-none absolute -left-8 top-8 h-32 w-32 rounded-full bg-[#2691C2]/20" />
          ...
          <SectionCard title="إجراءات المشرف" icon={<UserCheck className="h-3.5 w-3.5" />}>
            <div className="space-y-3">
              ...
              {selectedStatus === 'interview_scheduled' && (
                <div>
                  ...
                  <input
                    ...
                    className="h-11 w-full rounded-xl border border-[#22334A]/10 bg-white px-3.5 text-[13px] font-bold text-[#0F172A] outline-none focus:border-[#2691C2] focus:ring-2 focus:ring-[#2691C2]/15"
                  />
                </div>
              )}
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#EC943C] px-5 py-2.5 text-[12px] font-bold text-white shadow-sm transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40"
              >
            ...
            {canManage && (
              <form onSubmit={(e) => void handleAddNote(e)} className="space-y-3">
                <textarea
                  ...
                  rows={3}
                  className={`w-full resize-none rounded-xl border bg-white px-3.5 py-2.5 text-[13px] font-medium leading-relaxed text-[#0F172A] outline-none transition placeholder:text-slate-400 focus:ring-2 focus:ring-[#2691C2]/15 border-[#22334A]/10 focus:border-[#2691C2] ${SCROLL}`}
                />
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-[#EC943C] px-4 py-2 text-[11px] font-bold text-white shadow-sm transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40"
                  >
