...
const LOAD_ERROR = 'تعذّر تحميل التقييمات. تحقق من الاتصال وأعد المحاولة.'
...
async function fetchEvaluationRows(): Promise<EvaluationRow[]> {
  const list = await adminListEvaluations()
  return (list as EvaluationRow[]).map(normEval)
}
...
export default function AdminLmsEvaluationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      setRows(await fetchEvaluationRows())
    } catch {
      setError(LOAD_ERROR)
    } finally {
      setLoading(false)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const list = await fetchEvaluationRows()
