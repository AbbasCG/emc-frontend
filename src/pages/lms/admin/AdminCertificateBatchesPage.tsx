          ...
          <table className="w-full text-right text-sm">
            <thead>
              <tr className="border-b border-[#06182C]/[0.05] bg-[#06182C]/[0.02]">
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">رمز الإصدار</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">النوع</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">القالب</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">الحالة</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">الإجمالي</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">مُصدر / فشل</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">الإنشاء</th>
                <th className="px-4 py-3.5 text-[11px] font-black uppercase tracking-wide text-[#06182C]/40">الاكتمال</th>
                <th className="px-4 py-3.5" />
              </tr>
            </thead>
            <tbody className="divide-y divide-[#06182C]/[0.04]">
              {batches.map((b) => (
                <tr key={b.id} className="transition-colors hover:bg-[#06182C]/[0.015]">
                  <td className="px-4 py-3.5 font-mono text-[12px] font-bold text-[#06182C]">{b.batch_code}</td>
                  <td className="px-4 py-3.5">
                    <span className="inline-flex rounded-full bg-[#0077B6]/10 px-2.5 py-1 text-[11px] font-black text-[#0077B6]">
                      {CERT_TYPE_LABELS[b.certificate_type] ?? b.certificate_type}
                    </span>
                  </td>
                  <td className="px-4 py-3.5 max-w-[100px]">
                    <p className="truncate text-[11px] text-[#06182C]/50">{b.template?.name ?? '—'}</p>
                  </td>
                  <td className="px-4 py-3.5">
                  ...
                  <td className="px-4 py-3.5 font-bold text-[#06182C]">{fmtNum(b.total_recipients)}</td>
                  ...
                  <td className="px-4 py-3.5 text-[12px] text-[#06182C]/50 whitespace-nowrap">{fmtDate(b.created_at)}</td>
                  <td className="px-4 py-3.5 text-[12px] text-[#06182C]/50 whitespace-nowrap">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="rounded-xl bg-[#0077B6]/10 px-3 py-1.5 text-[11px] font-black text-[#0077B6] transition hover:bg-[#0077B6]/20"
                      >
                        التفاصيل
                      </button>
              ...
              {batches.length === 0 && !loading && (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-sm font-semibold text-[#06182C]/30">
          ...
          <>
            ...
            >
              <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-6 py-4">
                <h2 className="text-base font-black text-[#06182C]">
              ...
              {loadingDrawer ? (
                ...
                <div className="space-y-5 p-6">
                  {/* Batch summary */}
                  <div className="grid grid-cols-2 gap-3 rounded-2xl border border-[#0077B6]/20 bg-[#0077B6]/5 p-4">
                    <div>
                      <p className="text-[10px] font-black uppercase text-[#06182C]/40">الحالة</p>
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase text-[#06182C]/40">النوع</p>
                      <p className="mt-1 text-sm font-black text-[#06182C]">{CERT_TYPE_LABELS[drawerBatch.certificate_type]}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-black uppercase text-[#06182C]/40">الإجمالي</p>
                      <p className="mt-1 text-2xl font-black text-[#0077B6]">{fmtNum(drawerBatch.total_recipients)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-black uppercase text-[#06182C]/40">تم إصداره / فشل</p>
                  ...
                  {/* Certificates list */}
                  <h3 className="text-[12px] font-black uppercase tracking-wide text-[#06182C]/40">
                    ...
                    <table className="w-full text-right text-[12px]">
                      <thead>
                        <tr className="border-b border-slate-100 bg-slate-50">
                          <th className="px-3 py-2.5 font-black text-[#06182C]/50">المستلم</th>
                          <th className="px-3 py-2.5 font-black text-[#06182C]/50">الرمز</th>
                          <th className="px-3 py-2.5 font-black text-[#06182C]/50">الحالة</th>
                          <th className="px-3 py-2.5 font-black text-[#06182C]/50">التاريخ</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {drawerCerts.map((c) => (
                          <tr key={c.id} className="hover:bg-slate-50">
                            <td className="px-3 py-2.5 font-bold text-[#06182C]">
                              {c.recipient_name ?? c.user?.name ?? '—'}
                            </td>
                            <td className="px-3 py-2.5 font-mono text-[11px] text-[#06182C]/50">
                              {c.certificate_code ?? '—'}
                            </td>
                            ...
                            <td className="px-3 py-2.5 text-[#06182C]/40">{fmtDate(c.issued_at ?? c.created_at)}</td>
                          </tr>
                        ))}
                        {drawerCerts.length === 0 && (
                          <tr>
                            <td colSpan={4} className="py-6 text-center text-[#06182C]/30">لا توجد شهادات</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
