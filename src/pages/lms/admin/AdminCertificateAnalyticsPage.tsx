...
function Section({ title, icon: Icon, children, error, loading, action }: {
  ...
  return (
    <div className="rounded-2xl border border-[#0C2A4B]/10 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-[#0C2A4B]/8 px-5 py-4">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#0077B6]/10">
            <Icon size={16} className="text-[#0077B6]" />
          </div>
          <h2 className="text-[13px] font-black text-[#0C2A4B]">{title}</h2>
        </div>
        {action}
      </div>
      <div className="p-5">
        {loading ? (
          <div className="flex items-center justify-center py-10">
            <Loader2 size={22} className="animate-spin text-[#0077B6]/60" />
          </div>
        ) : error ? (
...
function KpiCard({ label, value, sub, color = C_BLUE, icon: Icon }: {
  ...
  return (
    <div className="rounded-2xl border border-[#0C2A4B]/10 bg-white p-4 shadow-sm">
      {Icon && (
      ...
      <p className="text-[10px] font-black uppercase tracking-wider text-[#0C2A4B]/45">{label}</p>
      <p className="mt-1 text-2xl font-black tabular-nums text-[#0C2A4B]">{fmt(typeof value === 'number' ? value : undefined) !== '—' ? value : value}</p>
      {sub && <p className="mt-0.5 text-[10px] font-medium text-[#0C2A4B]/50">{sub}</p>}
    </div>
  )
}
  ...
  return (
    <AdminLmsShell title="تحليلات الشهادات" description="مؤشرات الأداء التشغيلي لنظام الشهادات" breadcrumb="الشهادات / التحليلات">
      <div className="mx-auto max-w-[1400px] space-y-6 px-4 py-6" dir="rtl">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-1">
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl border border-[#0C2A4B]/15 bg-white px-3.5 py-2 text-[12px] font-black text-[#0C2A4B] shadow-sm transition hover:bg-[#f8fafc]"
            >
              <RefreshCw size={13} />
              تحديث
            </button>
          ...
          <div className="flex flex-wrap items-end gap-3">
                ...
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-[11px] font-black transition ${
                    filters.preset === p.value && !dateFrom
                      ...
                      : 'border border-[#0C2A4B]/15 bg-[#f8fafc] text-[#0C2A4B]/70 hover:bg-[#0077B6]/10'
                  }`}
                >
                  {p.label}
                </button>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="rounded-lg border border-[#0C2A4B]/15 px-2.5 py-1.5 text-[11px] font-semibold text-[#0C2A4B] focus:outline-none focus:ring-2 focus:ring-[#0077B6]/30"
              />
              <span className="text-[11px] text-[#0C2A4B]/40">إلى</span>
              <input
                type="date"
                value={dateTo}
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {loading['overview'] ? (
            Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="h-24 animate-pulse rounded-2xl bg-[#0C2A4B]/5" />
            ))
          ) : overview ? (
            <>
        ...
        {/* ── Tabs ─────────────────────────────────────────────────────────── */}
        <div className="flex gap-1 overflow-x-auto border-b border-[#0C2A4B]/10 pb-px">
          {TABS.map(t => (
            <button
              key={t.key}
              ...
              className={`shrink-0 rounded-t-lg px-4 py-2.5 text-[11px] font-black transition ${
                activeTab === t.key
                  ? 'border-b-2 border-[#0077B6] bg-[#0077B6]/8 text-[#0077B6]'
                  : 'text-[#0C2A4B]/55 hover:text-[#0C2A4B]'
              }`}
            >
              {t.label}
            </button>
        ...
        {activeTab === 'trends' && (
          <div className="space-y-6">
            <Section title="الشهادات الصادرة والمُنشأة بمرور الوقت" icon={TrendingUp}
              ...
              action={
                <button onClick={() => handleExport('trends')} disabled={!!exporting}
                  className="flex items-center gap-1.5 rounded-lg bg-[#0077B6]/10 px-3 py-1.5 text-[11px] font-black text-[#0077B6] transition hover:bg-[#0077B6]/20 disabled:opacity-50">
                  {exporting === 'trends' ? <Loader2 size={11} className="animate-spin" /> : <FileDown size={11} />}
                  تصدير
                </button>
              }>
              {trends.length === 0 ? (
                <p className="py-8 text-center text-[12px] text-[#0C2A4B]/40">لا توجد بيانات لهذه الفترة</p>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <AreaChart data={trends} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke="#0C2A4B08" />
                    <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#0C2A4B60' }} />
                    <YAxis tick={{ fontSize: 10, fill: '#0C2A4B60' }} />
                    <Tooltip
                      contentStyle={{ fontSize: 11, borderRadius: 8, border: '1px solid #0C2A4B10' }}
            ...
            <Section title="فشل PDF بمرور الوقت" icon={FileX} loading={loading['trends']} error={errors['trends']}>
              {trends.length === 0 ? (
                <p className="py-8 text-center text-[12px] text-[#0C2A4B]/40">لا توجد بيانات</p>
              ) : (
                  ...
                  <div className="flex flex-col gap-1.5">
                    {pieDistData.map((d, i) => (
                      <div key={i} className="flex items-center gap-2 text-[11px]">
                        ...
                        <span className="font-medium text-[#0C2A4B]/70">{d.name}</span>
                        <span className="mr-auto font-black tabular-nums text-[#0C2A4B]">{d.value.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            ...
            <Section title="أكثر معتمِدين نشاطاً" icon={Users} loading={loading['approval']} error={errors['approval']}>
              {!approval?.top_approvers.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد بيانات اعتماد</p>
              ) : (
                <div className="divide-y divide-[#0C2A4B]/6">
                  {approval.top_approvers.map((a, i) => (
                    <div key={a.user_id} className="flex items-center gap-3 py-2.5">
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#0077B6]/10 text-[10px] font-black text-[#0077B6]">{i + 1}</span>
                      <span className="flex-1 text-[12px] font-bold text-[#0C2A4B]">{a.name}</span>
                      <span className="text-[12px] font-black tabular-nums text-[#0C2A4B]">{a.count} اعتماد</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <Section title="اعتمادات متأخرة (أكثر من 7 أيام)" icon={AlertTriangle} loading={loading['approval']} error={errors['approval']}>
              {!approval?.stuck_approvals.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد اعتمادات متأخرة ✓</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-[11px]">
                    <thead>
                      <tr className="border-b border-[#0C2A4B]/8 text-[10px] font-black uppercase text-[#0C2A4B]/40">
                    ...
                    <tbody>
                      {approval.stuck_approvals.map(c => (
                        <tr key={c.id} className="border-b border-[#0C2A4B]/5 hover:bg-[#f8fafc]">
                          <td className="py-2 pr-2 font-mono font-bold text-[#0077B6]">{c.certificate_code}</td>
                          <td className="py-2 pr-2 text-[#0C2A4B]">{c.title}</td>
                          <td className="py-2 pr-2 text-[#0C2A4B]/70">{c.user?.name}</td>
                          ...
                          <td className="py-2 pr-2 text-[#0C2A4B]/60">{fmtDate(c.created_at)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="space-y-6">
            ...
            <Section title="أحدث حالات فشل PDF" icon={FileX} loading={loading['pdf']} error={errors['pdf']}>
              {!pdfMetrics?.latest_failed.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد حالات فشل ✓</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-[11px]">
                    <thead>
                    ...
                    <tbody>
                      {pdfMetrics.latest_failed.map(c => (
                        <tr key={c.id} className="border-b border-[#0C2A4B]/5 hover:bg-[#f8fafc]">
                          <td className="py-2 pr-2 font-mono text-[10px] font-bold text-[#0077B6]">{c.certificate_code}</td>
                          <td className="py-2 pr-2 text-[#0C2A4B]/70">{c.user?.name ?? '—'}</td>
                          <td className="max-w-[200px] py-2 pr-2">
                            <p className="truncate font-mono text-[10px] text-rose-600">{c.pdf_last_error ?? '—'}</p>
                          </td>
                          <td className="py-2 pr-2 text-[#0C2A4B]/60">{fmtDate(c.updated_at)}</td>
                          <td className="py-2 pr-2">
                            <button
                              onClick={() => navigate(`/dashboard/admin/certificates/${c.id}`)}
                              className="rounded-lg bg-[#0077B6]/10 px-2.5 py-1 text-[10px] font-black text-[#0077B6] transition hover:bg-[#0077B6]/20"
                            >
                              عرض
                            </button>
                          </td>
          ...
          <div className="space-y-6">
            ...
            <Section title="أكثر الشهادات تنزيلاً" icon={Download} loading={loading['downloads']} error={errors['downloads']}>
              {!downloads?.most_downloaded.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد تنزيلات</p>
              ) : (
                <div className="divide-y divide-[#0C2A4B]/6">
                  {downloads.most_downloaded.map((d, i) => (
                    <div key={d.certificate_id} className="flex items-center gap-3 py-2.5">
                      <span className="shrink-0 text-[11px] font-black tabular-nums text-[#0C2A4B]/40">{i + 1}</span>
                      <div className="flex-1 min-w-0">
                        <p className="truncate text-[12px] font-bold text-[#0C2A4B]">{d.title ?? '—'}</p>
                        <p className="font-mono text-[10px] text-[#0077B6]">{d.certificate_code}</p>
                      </div>
                      <span className="shrink-0 text-[12px] font-black tabular-nums text-[#0C2A4B]">{d.download_count} تنزيل</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <Section title="شهادات صادرة لم تُحمَّل بعد" icon={AlertTriangle} loading={loading['downloads']} error={errors['downloads']}>
              {!downloads?.not_downloaded.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">كل الشهادات الصادرة تم تحميلها ✓</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-[11px]">
                    <thead>
                    ...
                    <tbody>
                      {downloads.not_downloaded.map(c => (
                        <tr key={c.id} className="border-b border-[#0C2A4B]/5 hover:bg-[#f8fafc] cursor-pointer"
                          onClick={() => navigate(`/dashboard/admin/certificates/${c.id}`)}>
                          <td className="py-2 pr-2 font-mono text-[10px] font-bold text-[#0077B6]">{c.certificate_code}</td>
                          <td className="py-2 pr-2 text-[#0C2A4B]">{c.title}</td>
                          <td className="py-2 pr-2 text-[#0C2A4B]/60">{fmtDate(c.issued_at ?? '')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {activeTab === 'templates' && (
          <Section title="استخدام القوالب" icon={Layers}
            ...
            {!templateUsage.length ? (
              <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد قوالب</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-right text-[11px]">
                  <thead>
                  ...
                  <tbody>
                    {templateUsage.map(t => (
                      <tr key={t.id} className="border-b border-[#0C2A4B]/5 hover:bg-[#f8fafc]">
                        <td className="py-2.5 pr-2">
                          <div>
                            <p className="font-bold text-[#0C2A4B]">{t.name}</p>
                            {t.is_default && <span className="text-[9px] font-black text-[#0077B6]">افتراضي</span>}
                          </div>
                        </td>
                        <td className="py-2.5 pr-2">
                        ...
                        <td className="py-2.5 pr-2 font-black tabular-nums text-[#0C2A4B]">{fmtNum(t.total_certificates)}</td>
                        ...
                        <td className="py-2.5 pr-2 text-[#0C2A4B]/70">{t.avg_pdf_seconds != null ? `${t.avg_pdf_seconds}ث` : '—'}</td>
                        <td className="py-2.5 pr-2 text-[#0C2A4B]/60">{t.last_used_at ? fmtDate(t.last_used_at) : '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {activeTab === 'sources' && (
          <div className="space-y-6">
            <Section title="شهادات الدورات" icon={Award}
              ...
              {!courses.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد دورات بشهادات</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-[11px]">
                    <thead>
                    ...
                    <tbody>
                      {courses.slice(0, 30).map((c, i) => (
                        <tr key={i} className="border-b border-[#0C2A4B]/5 hover:bg-[#f8fafc]">
                          <td className="max-w-[200px] truncate py-2.5 pr-2 font-bold text-[#0C2A4B]">{c.course_title ?? c.path_title ?? '—'}</td>
                          <td className="py-2.5 pr-2 font-black tabular-nums text-[#0C2A4B]">{fmtNum(c.total)}</td>
          ...
          <div className="space-y-6">
            ...
            <Section title="الدُفعات الأخيرة" icon={Layers}
              ...
              {!batches?.batches.length ? (
                <p className="py-4 text-center text-[12px] text-[#0C2A4B]/40">لا توجد دُفعات</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-[11px]">
                    <thead>
                    ...
                    <tbody>
                      {batches.batches.map(b => (
                        <tr key={b.id} className="border-b border-[#0C2A4B]/5 hover:bg-[#f8fafc]">
                          <td className="py-2.5 pr-2 font-mono text-[10px] font-bold text-[#0077B6]">{b.batch_code}</td>
                          <td className="py-2.5 pr-2 text-[#0C2A4B]/70">{b.createdBy?.name ?? b.created_by_user?.name ?? '—'}</td>
                          <td className="py-2.5 pr-2">
                          ...
                          <td className="py-2.5 pr-2 font-black tabular-nums text-[#0C2A4B]">{b.total_recipients}</td>
                          ...
                          <td className="py-2.5 pr-2 text-[#0C2A4B]/60">{fmtDate(b.created_at)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <Section title="تقرير صحة العمليات" icon={AlertTriangle}
            ...
            {failures.length === 0 ? (
              <div className="flex flex-col items-center py-10">
                ...
                <p className="mt-3 text-[13px] font-black text-[#0C2A4B]">لا توجد أعطال ✓</p>
                <p className="mt-1 text-[11px] text-[#0C2A4B]/50">جميع العمليات تعمل بشكل طبيعي في هذه الفترة</p>
              </div>
            ) : (
                  ...
                  <div key={i} className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-[12px] font-black text-[#0C2A4B]">{f.label}</p>
                          <p className="text-[10px] font-medium text-[#0C2A4B]/55">{f.recommended_action}</p>
                        </div>
                      </div>
                      <div className="shrink-0 text-right">
                        <p className="text-xl font-black tabular-nums text-[#0C2A4B]">{f.count.toLocaleString()}</p>
                        {f.latest_occurrence && (
                          <p className="text-[10px] text-[#0C2A4B]/50">{fmtDate(f.latest_occurrence)}</p>
                        )}
                      </div>
                    </div>
                  </div>
