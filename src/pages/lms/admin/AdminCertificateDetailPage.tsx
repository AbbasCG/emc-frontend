  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-2.5" dir="rtl">
        <p className="text-[11px] font-black uppercase tracking-wide text-[#0d1b2a]/30">معاينة الشهادة</p>
        <div className="flex items-center gap-1">
          <button
            type="button"
            title="تصغير"
            onClick={() => changeZoom(-0.1)}
            className="flex h-7 w-7 items-center justify-center rounded-lg text-[#0d1b2a]/40 transition hover:bg-slate-100 hover:text-[#0d1b2a]"
          >
            <ZoomOut size={13} />
          </button>
          <span className="min-w-[3rem] text-center text-[11px] font-black tabular-nums text-[#0d1b2a]/50">
            {Math.round(zoom * 100)}%
          </span>
          <button
            type="button"
          ...
          <button
            ...
            title="ملاءمة العرض"
            onClick={fitWidth}
            className="flex h-7 items-center gap-1 rounded-lg px-2 text-[11px] font-black text-[#0d1b2a]/40 transition hover:bg-slate-100 hover:text-[#0d1b2a]"
          >
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3" dir="rtl">
                <p className="text-[12px] font-black text-[#0d1b2a]">معاينة الشهادة</p>
  ...
  return (
    <div className="flex flex-col gap-1">
      <button
        ...
        disabled={busy}
        className="flex items-center gap-2 rounded-xl bg-[#2691C2] px-4 py-2.5 text-[12px] font-black text-white shadow-sm transition hover:bg-[#1e7aaa] disabled:opacity-50"
      >
  ...
  return (
    <LmsDataPanel>
      <div className="p-5" dir="rtl">
        <p className="mb-4 text-[11px] font-black uppercase tracking-wide text-[#0d1b2a]/30">{title}</p>
        {children}
      </div>
    </LmsDataPanel>
  )
...
function Field({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div>
      <p className="text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/30">{label}</p>
      <p className="mt-0.5 text-[13px] font-bold text-[#0d1b2a]">{value}</p>
    </div>
  )
}
      ...
      {!loading && cert && (
        <div className="space-y-5" dir="rtl">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4 p-5">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-[#0d1b2a]/5 px-3 py-1 text-[11px] font-black text-[#0d1b2a]/50">
                    {CERT_TYPE_LABELS[cert.certificate_type as keyof typeof CERT_TYPE_LABELS] ?? cert.certificate_type}
                  </span>
                </div>
                <h1 className="text-xl font-black text-[#0d1b2a]">{cert.title}</h1>
                {cert.certificate_code && (
                  <p className="font-mono text-[12px] text-[#0d1b2a]/40">{cert.certificate_code}</p>
                )}
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  disabled={actionBusy}
                  className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-[12px] font-black text-[#0d1b2a]/60 transition hover:border-slate-300 hover:bg-slate-50 disabled:opacity-50"
                >
            ...
            <div className="space-y-5 lg:col-span-2">
              ...
              <Section title="المستلم">
                <div className="flex items-start gap-4">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#2691C2]/10">
                    <User size={18} className="text-[#2691C2]" />
                  </div>
          ...
          {cert.pdf_info && (
            <Section title="معلومات ملف PDF">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/30">حالة الملف</p>
                  <div className="mt-1">
                ...
                {cert.pdf_info.generated_at && (
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/30">تاريخ الإنشاء</p>
                    <p className="mt-0.5 text-[13px] font-bold text-[#0d1b2a]">{fmtDate(cert.pdf_info.generated_at)}</p>
                  </div>
                )}
                {cert.pdf_info.file_size_bytes != null && (
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/30">حجم الملف</p>
                    <p className="mt-0.5 text-[13px] font-bold text-[#0d1b2a]">
                ...
                {cert.pdf_info.template_id_used != null && (
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/30">القالب المستخدم</p>
                    <p className="mt-0.5 text-[13px] font-bold text-[#0d1b2a]">#{cert.pdf_info.template_id_used}</p>
                  </div>
                )}
              </div>
                  ...
                  <div key={i} className={`rounded-xl p-4 ring-1 ${
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-[12px] font-black text-[#0d1b2a]">{approval.type_label}</p>
                          {approval.approver && (
                            <p className="text-[11px] text-[#0d1b2a]/50">{approval.approver.name}</p>
                          )}
                        </div>
                      </div>
                      <div className="text-left">
                        ...
                        {approval.actioned_at && (
                          <p className="mt-0.5 text-[10px] text-[#0d1b2a]/40">{fmtDate(approval.actioned_at)}</p>
                        )}
                      </div>
                    </div>
                    {approval.notes && (
                      <p className="mt-2 text-[12px] text-[#0d1b2a]/60 border-t border-black/5 pt-2">{approval.notes}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <Section title="سجل النشاط">
            {logsLoading ? (
              <div className="flex items-center gap-2 py-4 text-[12px] font-bold text-[#0d1b2a]/40">
                <Loader2 size={14} className="animate-spin" /> جارٍ تحميل السجل...
              </div>
            ) : logs.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-6">
                <History size={24} className="text-[#0d1b2a]/15" />
                <p className="text-[12px] font-bold text-[#0d1b2a]/30">لا توجد إجراءات مسجّلة</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-right text-[12px]">
                  <thead>
                    <tr className="border-b border-[#0d1b2a]/[0.05]">
                      <th className="pb-2 pl-3 font-black text-[10px] uppercase tracking-wide text-[#0d1b2a]/30">الإجراء</th>
                      <th className="pb-2 pl-3 font-black text-[10px] uppercase tracking-wide text-[#0d1b2a]/30">المسؤول</th>
                      <th className="pb-2 pl-3 font-black text-[10px] uppercase tracking-wide text-[#0d1b2a]/30">عنوان IP</th>
                      <th className="pb-2 font-black text-[10px] uppercase tracking-wide text-[#0d1b2a]/30">التاريخ</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#0d1b2a]/[0.04]">
                    {logs.map(log => (
                      <tr key={log.id} className="hover:bg-[#0d1b2a]/[0.015]">
                        <td className="py-2 pl-3 font-mono text-[11px] text-[#2691C2]">{log.action}</td>
                        <td className="py-2 pl-3 font-semibold text-[#0d1b2a]/60">{log.actor?.name ?? '—'}</td>
                        <td className="py-2 pl-3 font-mono text-[10px] text-[#0d1b2a]/40">{log.ip_address ?? '—'}</td>
                        <td className="py-2 whitespace-nowrap text-[11px] text-[#0d1b2a]/40">{fmtDate(log.created_at)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
