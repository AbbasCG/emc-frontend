...
function Chip({ count, label, variant }: { count: number; label: string; variant: 'blue' | 'amber' | 'emerald' | 'rose' }) {
  const cls = {
    blue: 'bg-[#0077B6]/[0.07] text-[#0077B6] ring-[#0077B6]/10',
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" role="dialog" aria-modal>
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-[#0C2A4B]/[0.08]" dir="rtl">
        <div className="flex items-center justify-between border-b border-[#0C2A4B]/[0.06] px-6 py-4">
          ...
          <button onClick={onClose} className="rounded-lg p-1.5 text-[#0C2A4B]/40 hover:bg-slate-100 hover:text-[#0C2A4B]" aria-label="إغلاق">
            <X size={16} />
          </button>
        </div>
        <div className="max-h-[70vh] overflow-y-auto">
          <div className="space-y-4 px-6 py-5">
            {/* Title */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">عنوان الواجب <span className="text-rose-500">*</span></label>
              <input
                value={form.title}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">أقصى درجة</label>
                <input
                  type="number"
...
function AssignmentCard({
  ...
  return (
    <motion.button
      ...
      onClick={onOpen}
      whileHover={{ y: -2 }}
      className="group w-full rounded-2xl border border-[#0C2A4B]/8 bg-white p-4 text-right shadow-sm transition hover:border-[#0077B6]/25 hover:shadow-md"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="font-black text-[#0C2A4B] line-clamp-1 group-hover:text-[#0077B6]">{row.title ?? '—'}</p>
          <p className="mt-0.5 text-[12px] font-semibold text-[#0077B6]/80 line-clamp-1">{row.course_title}</p>
          <p className="mt-1 text-[11px] font-medium text-[#0C2A4B]/45">{row.instructor_name}</p>
        </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 border-t border-[#0C2A4B]/6 pt-3">
        <div className="text-[11px] font-bold text-[#0C2A4B]/50">
          <span className="text-[#0C2A4B]/35">التسليم: </span>
          <span className={overdue ? 'text-rose-600' : 'text-[#0C2A4B]'}>{formatLmsDateTime(row.due_date ?? row.due_at)}</span>
          {overdue ? <span className="mr-1 text-rose-600">· متأخر</span> : null}
        </div>
        {row.max_score != null ?
          <span className="text-[11px] font-black text-[#0C2A4B]/45">{fmtNum(row.max_score)} نقطة</span>
        : null}
      </div>
...
function AssignmentCardSkeleton() {
  return (
    <div className="rounded-2xl border border-[#0C2A4B]/6 bg-white p-4 shadow-sm">
  ...
  const toggleBtn = (active: boolean) =>
    `rounded-xl border px-3 py-2 text-[12px] font-black transition ${
      active
        ...
        : 'border-[#0C2A4B]/10 bg-white text-[#0C2A4B]/55 hover:border-[#0077B6]/20'
    }`
      ...
      >
        ...
        : (
          <LmsDataPanel footer={`عرض ${fmtNum(filtered.length)} من ${fmtNum(total)} واجب`}>
            <div className="hidden border-b border-[#0C2A4B]/6 bg-[#f8fafc]/80 px-5 py-3 lg:grid lg:grid-cols-[1.4fr_1fr_0.8fr_1.2fr_0.6fr] lg:gap-3">
              {['الواجب / الدورة', 'المدرب', 'التسليم', 'التسليمات', 'الحالة'].map((h) => (
                <span key={h} className="text-[10px] font-black uppercase tracking-wide text-[#0C2A4B]/40">{h}</span>
              ))}
            </div>
            <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-1 xl:gap-0 xl:divide-y xl:divide-[#0C2A4B]/6">
              {filtered.map((r) => (
                <div key={r.id} className="xl:px-2 xl:py-1">
                  <div className="hidden xl:block">
                    <motion.button
                      ...
                      whileHover={{ backgroundColor: 'rgba(0,119,182,0.04)' }}
                    ...
                    >
                      <div className="min-w-0">
                        <p className="font-bold text-[#0C2A4B] line-clamp-1">{r.title ?? '—'}</p>
                        <p className="text-[11px] font-semibold text-[#0077B6]/70 line-clamp-1">{r.course_title}</p>
                      </div>
                      <p className="text-[12px] font-semibold text-[#0C2A4B]/65">{r.instructor_name}</p>
                      <div>
                        <p className={`text-[12px] font-semibold ${isOverdue(r.due_date) && r.status === 'active' ? 'text-rose-600' : 'text-[#0C2A4B]/65'}`}>
                          {formatLmsDateTime(r.due_date ?? r.due_at)}
                        </p>
                        {r.max_score != null ?
                          <p className="text-[10px] text-[#0C2A4B]/35">{fmtNum(r.max_score)} نقطة</p>
                        : null}
                      </div>
