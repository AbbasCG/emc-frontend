      ...
      <LmsDataPanel>
        <div className="px-5 pt-4 pb-2">
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div dir="rtl">
              <h2 className="text-sm font-black text-[#0d1b2a]">الشهادات</h2>
              <p className="mt-0.5 text-[11px] text-[#0d1b2a]/40">
                {listLoading ? '...' : `${fmtNum(total)} شهادة`}
                {hasActiveFilters && <span className="mr-1 text-[#2691C2]">· فلاتر مفعّلة</span>}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-1.5 text-[11px] font-black text-[#0d1b2a]/50 transition hover:border-emerald-200 hover:text-emerald-700 disabled:opacity-50"
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-[11px] font-black transition ${hasActiveFilters ? 'border-[#2691C2] bg-[#2691C2]/5 text-[#2691C2]' : 'border-slate-200 text-[#0d1b2a]/50 hover:border-[#2691C2]/30 hover:text-[#2691C2]'}`}
              >
                <Filter size={12} />
          ...
          <div className="mb-3 flex gap-2" dir="rtl">
            <div className="relative flex-1">
              <Search size={13} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#0d1b2a]/30" />
              <input
                ref={searchRef}
                type="text"
                value={draft.search}
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pr-9 pl-3 text-[12px] text-[#0d1b2a] placeholder:text-[#0d1b2a]/30 focus:border-[#2691C2]/40 focus:bg-white focus:outline-none"
              />
            </div>
            <button
              type="button"
              onClick={applyFilters}
              className="rounded-xl bg-[#2691C2] px-4 py-2 text-[11px] font-black text-white transition hover:bg-[#1e7aaa]"
            >
              بحث
            </button>
            {hasActiveFilters && (
              <button
                ...
                className="flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-[11px] font-black text-[#0d1b2a]/50 transition hover:border-rose-200 hover:text-rose-600"
              >
          ...
          {filtersOpen && (
            <div className="mb-4 rounded-xl border border-[#2691C2]/10 bg-[#2691C2]/[0.03] p-4" dir="rtl">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">

                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">الحالة</span>
                  <select value={draft.status} onChange={e => setDraft(d => ({ ...d, status: e.target.value }))}
                    className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[12px] text-[#0d1b2a] focus:border-[#2691C2]/40 focus:outline-none">
                ...
                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">نوع الشهادة</span>
                ...
                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">حالة الاعتماد</span>
                ...
                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">حالة ملف PDF</span>
                ...
                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">من تاريخ الإصدار</span>
                  <input type="date" value={draft.issued_from} onChange={e => setDraft(d => ({ ...d, issued_from: e.target.value }))}
                    className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[12px] text-[#0d1b2a] focus:border-[#2691C2]/40 focus:outline-none" />
                </label>

                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">إلى تاريخ الإصدار</span>
                  <input type="date" value={draft.issued_to} onChange={e => setDraft(d => ({ ...d, issued_to: e.target.value }))}
                    className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[12px] text-[#0d1b2a] focus:border-[#2691C2]/40 focus:outline-none" />
                </label>

                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">من تاريخ الإنشاء</span>
                  <input type="date" value={draft.created_from} onChange={e => setDraft(d => ({ ...d, created_from: e.target.value }))}
                    className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[12px] text-[#0d1b2a] focus:border-[#2691C2]/40 focus:outline-none" />
                </label>

                <label className="grid gap-1">
                  <span className="text-[11px] font-black text-[#0d1b2a]/50">إلى تاريخ الإنشاء</span>
              ...
              <div className="mt-4 flex justify-end gap-2">
                <button type="button" onClick={resetFilters}
                  className="rounded-xl border border-slate-200 px-4 py-2 text-[11px] font-black text-[#0d1b2a]/50 transition hover:border-rose-200 hover:text-rose-600">
                  إعادة تعيين
                </button>
                <button type="button" onClick={applyFilters}
                  className="rounded-xl bg-[#2691C2] px-5 py-2 text-[11px] font-black text-white transition hover:bg-[#1e7aaa]">
                  تطبيق الفلاتر
                </button>
              </div>
            </div>
              ...
              <table className="w-full text-right text-sm">
                <thead>
                  <tr className="border-b border-[#0d1b2a]/[0.05] bg-[#0d1b2a]/[0.02]">
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">رقم الشهادة</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">الطالب</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">النوع</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">الدورة / الورشة</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">القالب</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">الحالة</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">PDF</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">التاريخ</th>
                    <th className="px-3 py-3 text-[10px] font-black uppercase tracking-wide text-[#0d1b2a]/40">إجراءات</th>
                  </tr>
                </thead>
                    ...
                    : certs.map((cert: Certificate) => {
                        ...
                        return (
                          <tr key={cert.id} className="transition-colors hover:bg-[#0d1b2a]/[0.015]">
                            <td className="px-3 py-3 font-mono text-[10px] text-[#0d1b2a]/50">
                              {cert.certificate_code ?? '—'}
                            </td>
                            <td className="px-3 py-3 max-w-[140px]">
                              <p className="truncate font-bold text-[12px] text-[#0d1b2a]">{cert.recipient_name ?? cert.user?.name ?? '—'}</p>
                              <p className="truncate text-[10px] text-[#0d1b2a]/40">{cert.recipient_email ?? cert.user?.email ?? ''}</p>
                            </td>
                            ...
                            <td className="px-3 py-3 max-w-[140px]">
                              <p className="truncate text-[11px] font-semibold text-[#0d1b2a]/70">{related}</p>
                            </td>
                            <td className="px-3 py-3 max-w-[110px]">
                              <p className="truncate text-[11px] text-[#0d1b2a]/50">{cert.template?.name ?? '—'}</p>
                            </td>
                            <td className="px-3 py-3">
                            ...
                            <td className="px-3 py-3 text-[11px] font-semibold text-[#0d1b2a]/50 whitespace-nowrap">
                            ...
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  className="flex items-center gap-1 rounded-lg bg-[#2691C2]/10 px-2 py-1 text-[10px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/20"
                                >
                                  <Eye size={10} /> عرض
                                </button>
                    ...
                    <tr>
                      <td colSpan={9} className="px-4 py-10 text-center" dir="rtl">
                        <div className="flex flex-col items-center gap-2">
                          <Award size={28} className="text-[#0d1b2a]/15" />
                          <p className="text-sm font-bold text-[#0d1b2a]/30">لا توجد شهادات تطابق هذه الفلاتر</p>
                          {hasActiveFilters && (
                            <button type="button" onClick={resetFilters}
          ...
          {!listLoading && lastPage > 1 && (
            <div className="mt-4 flex items-center justify-between border-t border-[#0d1b2a]/[0.05] pt-4" dir="rtl">
              <p className="text-[11px] font-semibold text-[#0d1b2a]/40">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-[#0d1b2a]/50 transition hover:border-[#2691C2]/30 hover:text-[#2691C2] disabled:opacity-30"
                >
                ...
                {Array.from({ length: Math.min(lastPage, 7) }, (_, i) => {
                  ...
                  return (
                    <button key={pg} type="button" onClick={() => setPage(pg)}
                      className={`flex h-8 w-8 items-center justify-center rounded-lg text-[11px] font-black transition ${pg === page ? 'bg-[#2691C2] text-white' : 'border border-slate-200 text-[#0d1b2a]/50 hover:border-[#2691C2]/30 hover:text-[#2691C2]'}`}>
                      {pg}
                    </button>
                  )
                })}
