    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" role="dialog" aria-modal>
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-[#22334A]/[0.08]" dir="rtl">
        ...
        <div className="flex items-center justify-between border-b border-[#22334A]/[0.06] px-6 py-4">
          <h2 className="text-[15px] font-black text-[#22334A]">{isEdit ? 'تعديل المادة' : 'إضافة مادة تعليمية'}</h2>
          <button type="button" onClick={onClose} className="rounded-lg p-1.5 text-[#22334A]/40 hover:bg-slate-100">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          <div className="space-y-4 px-6 py-5">
            {/* Title */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#22334A]/60">عنوان المادة <span className="text-rose-500">*</span></label>
              <input
                value={form.title}
            ...
            {/* Type */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#22334A]/60">نوع المادة</label>
            ...
            {form.type !== 'link' && form.type !== 'programming_project' && (
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 rounded-xl border py-2 text-[12px] font-black transition ${form.source === 'file' ? 'border-[#2691C2] bg-[#2691C2]/10 text-[#2691C2]' : 'border-[#22334A]/10 text-[#22334A]/50 hover:border-[#22334A]/20'}`}
                >
                  رفع ملف
                </button>
                <button
                  ...
                  className={`flex-1 rounded-xl border py-2 text-[12px] font-black transition ${form.source === 'url' ? 'border-[#2691C2] bg-[#2691C2]/10 text-[#2691C2]' : 'border-[#22334A]/10 text-[#22334A]/50 hover:border-[#22334A]/20'}`}
                >
                  رابط خارجي
                </button>
              </div>
            ...
            {form.source === 'file' && form.type !== 'link' && (
              <div>
                <label className="mb-1.5 block text-[12px] font-black text-[#22334A]/60">الملف <span className="text-rose-500">*</span></label>
                <input
                  ref={fileRef}
                  type="file"
                ...
                <button
                  ...
                  className={[
                    ...
                    form.file ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-[#22334A]/15 text-[#22334A]/40 hover:border-[#2691C2]/30 hover:text-[#2691C2]',
                  ].join(' ')}
                >
                  {form.file ? (
            ...
            {(form.source === 'url' || form.type === 'link') && (
              <div>
                <label className="mb-1.5 block text-[12px] font-black text-[#22334A]/60">الرابط <span className="text-rose-500">*</span></label>
                <input
...
function MaterialCard({
  ...
  return (
    ...
    >
      <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${t.bg} ring-1 ring-[#22334A]/6`}>
        <TypeIcon size={18} className={t.text} aria-hidden />
      </span>
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <p className="line-clamp-1 text-sm font-bold text-[#22334A]">{m.title ?? '—'}</p>
          <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-black ring-1 ring-[#22334A]/10 ${t.bg} ${t.text}`}>
            {t.label}
          </span>
        </div>
        <p className="mt-0.5 line-clamp-1 text-xs font-semibold text-[#2691C2]/80">{m.course_title}</p>
        {m.description ?
          <p className="mt-1 line-clamp-2 text-xs text-[#22334A]/40">{m.description}</p>
        : null}
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <p className="text-[11px] font-medium text-[#22334A]/35">{fmtDate(m.created_at ?? m.updated_at)}</p>
            {size ? <span className="font-latin text-[11px] font-medium text-[#22334A]/25">{size}</span> : null}
            {m.is_visible === false ?
        ...
        <div className="flex items-center gap-2">
              ...
              <>
                {m.can_preview ?
                  <button
                    ...
                    onClick={() => onPreview(m)}
                    disabled={isBusy}
                    className="flex items-center gap-1 rounded-lg bg-[#2691C2]/10 px-2.5 py-1 text-[11px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/20 disabled:opacity-50"
                  >
                    {isPreviewBusy ?
                      <span className="inline-flex items-center gap-1">
                        <span className="h-3 w-3 animate-spin rounded-full border-2 border-[#2691C2]/30 border-t-[#2691C2]" />
                        جاري الفتح…
                      </span>
                    : <><Eye size={11} aria-hidden /> معاينة</>}
                  </button>
                : (
                  <span className="rounded-lg bg-[#22334A]/4 px-2.5 py-1 text-[10px] font-bold text-[#22334A]/35">
                ...
                <button
                  ...
                  disabled={isBusy}
                  className="flex items-center gap-1 rounded-lg bg-[#22334A]/6 px-2.5 py-1 text-[11px] font-black text-[#22334A] transition hover:bg-[#22334A]/10 disabled:opacity-50"
                >
                  {isDownloadBusy ?
                    <span className="inline-flex items-center gap-1">
                      <span className="h-3 w-3 animate-spin rounded-full border-2 border-[#22334A]/20 border-t-[#22334A]" />
                      جاري التحميل…
                    </span>
            ...
            : !m.external_url ?
              <span className="text-[10px] text-[#22334A]/30">لا يوجد ملف مرفق</span>
            : null}
          </div>
        </div>
    </motion.div>
      ...
      <AdminLmsShell
        ...
        action={
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-[#2691C2] px-4 py-2 text-[12px] font-black text-white shadow hover:bg-[#2691C2]/90"
          >
            <Plus size={14} />
            إضافة مادة
          </button>
