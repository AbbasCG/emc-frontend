      ...
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-[#22334A]/[0.08]" dir="rtl">
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          <div className="space-y-4 px-6 py-5">
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#22334A]/60">عنوان الجلسة <span className="text-rose-500">*</span></label>
              <input
                value={form.title}
            ...
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#22334A]/60">رابط الاجتماع</label>
              <input
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        {/* Title */}
        <div>
          <h3 className="line-clamp-2 text-[15px] font-black leading-snug text-[#22334A]">
        ...
        <div className="mt-auto flex items-center gap-3 border-t border-slate-50 pt-3 text-[11px] font-bold">
          ...
          {hasLink && (
            <span className="flex items-center gap-1 text-[#2691C2]">
              <Link2 size={11} />
              رابط
            </span>
          )}
      ...
      <div className="flex gap-2 border-t border-slate-100 bg-slate-50/60 px-5 py-3">
        <button
          ...
          className="flex-1 rounded-xl border border-slate-200 bg-white py-2 text-[11px] font-black text-slate-700 transition hover:border-[#2691C2]/30 hover:text-[#2691C2]"
        >
          التفاصيل
        </button>
        {!isBlocked && hasLink && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex-1 rounded-xl bg-[#2691C2] py-2 text-center text-[11px] font-black text-white transition hover:bg-[#2691C2]/90"
          >
            فتح الرابط
          </a>
        )}
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-100 bg-white px-6 py-5">
          <div>
            ...
            <h2 className="mt-1 text-base font-black leading-snug text-[#22334A]">
              {row.title ?? normCourseTitle(row.course_title ?? row.course_name)}
            </h2>
          </div>
          <button
        ...
        <div className="space-y-6 px-6 py-6">
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50/60 divide-y divide-slate-100">
              {row.course_title && (
                <div className="flex items-center gap-3 px-4 py-3 text-sm">
                  <BookOpen size={14} className="shrink-0 text-[#2691C2]" />
                  <div>
                    <p className="text-[10px] font-bold text-slate-400">الدورة</p>
                    <p className="font-black text-[#22334A]">{normCourseTitle(row.course_title)}</p>
                  </div>
                </div>
              )}
              {row.instructor_name && (
                <div className="flex items-center gap-3 px-4 py-3 text-sm">
                  <GraduationCap size={14} className="shrink-0 text-[#2691C2]" />
                  <div>
                    <p className="text-[10px] font-bold text-slate-400">المدرب</p>
                    <p className="font-black text-[#22334A]">{normInstructor(row.instructor_name)}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <section>
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50/60 divide-y divide-slate-100">
              <div className="flex items-center gap-3 px-4 py-3 text-sm">
                <Calendar size={14} className="shrink-0 text-[#2691C2]" />
                <div>
                  <p className="text-[10px] font-bold text-slate-400">التاريخ</p>
                  <p className="font-black text-[#22334A]">{fmtDate(deriveSessionDate(row)) || '—'}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 px-4 py-3 text-sm">
          ...
          <section>
            ...
            {row.meeting_link ?? row.meeting_url ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 rounded-xl border border-[#2691C2]/20 bg-[#2691C2]/5 px-4 py-3 text-sm font-black text-[#2691C2] transition hover:bg-[#2691C2]/10"
              >
                <Link2 size={14} />
                فتح رابط الاجتماع
              </a>
          ...
          <section className="grid grid-cols-3 gap-3">
            {[
              { label: 'الحضور', value: fmtNum(row.attendance_count ?? 0), icon: Users, color: 'text-[#2691C2]' },
    ...
    >
      ...
      <div className="mb-6 space-y-3" dir="rtl">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative min-w-[220px] flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white py-2 pr-9 pl-3 text-[12px] font-medium text-slate-800 placeholder:text-slate-400 focus:border-[#2691C2] focus:outline-none shadow-sm"
            />
          </div>
        ...
        <p className="text-[12px] font-bold text-slate-500">
          {loading ? (
            ...
            <>عرض <span className="font-black text-[#22334A]">{fmtNum(filtered.length)}</span> من <span className="font-black text-[#22334A]">{fmtNum(total)}</span> جلسة</>
          )}
        </p>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-white py-20 text-center">
          ...
          {hasActiveFilters && (
            <button type="button" onClick={clearFilters} className="mt-3 text-sm font-bold text-[#2691C2] hover:underline">
              إعادة تعيين الفلاتر
            </button>
          )}
        </div>
