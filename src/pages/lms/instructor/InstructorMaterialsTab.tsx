          ...
          {filtered.map((m) => (
            <div key={m.id} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                {m.preview_url && (
                  <button type="button" onClick={() => handlePreview(m)} className="rounded-lg border border-deepBlue/10 p-2 text-deepBlue/60 hover:border-[#0077B6]/30"><Eye className="h-3.5 w-3.5" /></button>
                )}
                {m.download_url && (
                  <button type="button" onClick={() => handleDownload(m)} className="rounded-lg border border-deepBlue/10 p-2 text-deepBlue/60 hover:border-[#0077B6]/30"><Download className="h-3.5 w-3.5" /></button>
                )}
                {canManage && (
                  <>
          ...
          <div dir="rtl" className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select value={type} onChange={(e) => setType(e.target.value)} className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#0077B6]">
                  {Object.entries(MATERIAL_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                </select>
              </div>
              {type === 'link' ? (
                <div>
                  ...
                  <input value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} dir="ltr" className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#0077B6]" />
              ...
              <div>
                ...
                <select value={scopeChoice} onChange={(e) => setScopeChoice(e.target.value as typeof scopeChoice)}
                  className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#0077B6]">
              ...
              {scopeChoice === 'lesson' && (
                <div>
                  ...
                  <select value={lessonId} onChange={(e) => setLessonId(e.target.value ? Number(e.target.value) : '')}
                    disabled={!moduleId}
                    className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#0077B6] disabled:opacity-50">
