        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" disabled={!!exporting} onClick={() => handleExport('csv')}
              className="inline-flex items-center gap-1.5 rounded-xl border border-deepBlue/10 bg-white px-3 py-2 text-[11px] font-bold text-deepBlue/70 transition hover:border-[#2691C2]/30 disabled:opacity-40">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <select value={filters.status ?? ''} onChange={(e) => updateFilter('status', e.target.value || undefined)}
              className="mt-1 block rounded-xl border border-slate-200 px-3 py-2 text-[11px] font-bold text-deepBlue outline-none focus:border-[#2691C2]">
          ...
          <div>
            ...
            <input type="number" value={filters.course_id ?? ''} onChange={(e) => updateFilter('course_id', e.target.value ? Number(e.target.value) : undefined)}
              placeholder="اختياري"
              className="mt-1 block w-28 rounded-xl border border-slate-200 px-3 py-2 text-[11px] font-bold text-deepBlue outline-none focus:border-[#2691C2]" />
          </div>
