          ...
          <div dir="rtl" className="w-full max-w-md rounded-2xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#2691C2]" />
                <FieldError errors={errors} field="description" />
              </div>
              <div>
                <label className="text-[10px] font-black text-deepBlue/40">الحالة</label>
                <select value={status} onChange={(e) => setStatus(e.target.value as ContentStatus)} className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#2691C2]">
    ...
    <div dir="rtl">
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <select value={selectedModuleId ?? ''} onChange={(e) => setSelectedModuleId(Number(e.target.value))}
          className="rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#2691C2]">
        ...
        <div className="space-y-2">
          {lessons.map((l, i) => (
            <div key={l.id} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4">
              ...
              {canManage && (
                <div className="flex shrink-0 items-center gap-1.5">
                  <button type="button" onClick={() => openEdit(l)} className="rounded-lg border border-deepBlue/10 p-2 text-deepBlue/60 hover:border-[#2691C2]/30"><Pencil className="h-3.5 w-3.5" /></button>
          ...
          <div dir="rtl" className="w-full max-w-md rounded-2xl bg-white p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} dir="ltr" className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#2691C2]" />
                <FieldError errors={errors} field="video_url" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-black text-deepBlue/40">المدة (دقيقة)</label>
                  <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#2691C2]" />
                </div>
                <div>
                  <label className="text-[10px] font-black text-deepBlue/40">الحالة</label>
                  <select value={status} onChange={(e) => setStatus(e.target.value as ContentStatus)} className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-bold text-deepBlue outline-none focus:border-[#2691C2]">
        ...
        <div className="space-y-3">
          {modules.sort((a, b) => a.sort_order - b.sort_order).map((m) => (
            <div key={m.id} className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="flex items-center gap-2">
                <Layers className="h-4 w-4 text-[#2691C2]" />
