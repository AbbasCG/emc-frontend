...
function AttendanceCard({ row }: { row: StudentAttendanceRecord }) {
  ...
  return (
    <article className="flex flex-col gap-3 rounded-2xl border border-[#0C2A4B]/[0.07] bg-white p-4 shadow-sm transition hover:shadow-md">
      ...
      {when !== '—' && (
        <div className="flex items-center gap-2 rounded-xl border border-[#0C2A4B]/[0.07] bg-slate-50/70 px-3 py-2">
          <Clock className="h-3.5 w-3.5 shrink-0 text-[#0C2A4B]/40" />
          <p className="text-[12px] font-bold tabular-nums text-[#0C2A4B]">{when}</p>
        </div>
      )}

      {row.notes?.trim() && (
        <p className="rounded-xl border border-[#0C2A4B]/[0.06] bg-slate-50/50 px-3 py-2 text-[12px] font-medium leading-relaxed text-[#0F172A]/70">
          {row.notes}
        </p>
      )}
    </article>
  ...
  return (
    <div className="space-y-6 pb-10 text-right" dir="rtl">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4 rounded-3xl border border-[#0C2A4B]/[0.06] bg-gradient-to-bl from-white/95 to-[#0077B6]/[0.03] p-6 shadow-sm ring-1 ring-[#0C2A4B]/[0.04]">
        <div>
          <h1 className="text-2xl font-black text-[#0C2A4B]">سجل الحضور</h1>
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard label="إجمالي الجلسات" value={stats.total} colorClass="border-[#0C2A4B]/[0.07] bg-white text-[#0C2A4B]" />
      ...
      {summary && summary.total > 0 && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard label="أطول تتابع حضور" value={summary.longest_attendance_streak} colorClass="border-[#0C2A4B]/[0.07] bg-white text-[#0C2A4B]" />
          <StatCard
            label="مستوى الخطر"
