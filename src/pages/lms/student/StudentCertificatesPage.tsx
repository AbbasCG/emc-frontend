...
function DownloadCardBtn({ cert, large }: { cert: Certificate; large?: boolean }) {
  ...
  return (
    <button
      ...
      onClick={() => void handle()}
      className={large
        ? `flex items-center gap-2 rounded-xl px-5 py-2 text-sm font-black text-white transition disabled:opacity-50 ${isError ? 'bg-rose-500 hover:bg-rose-600' : 'bg-[#2691C2] hover:bg-[#1e7aaa]'}`
        : `flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-[11px] font-black text-white transition disabled:opacity-50 ${isError ? 'bg-rose-500 hover:bg-rose-600' : 'bg-[#2691C2] hover:bg-[#1e7aaa]'}`
      }
    >
      {busy
    ...
    <div dir="rtl" className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm font-semibold text-slate-500">
          ...
          {total > 0 && <span className="mr-2 text-[#2691C2]">({total})</span>}
        </p>
      </header>
            ...
            {certs.map((cert, i) => {
              ...
              return (
                ...
                >
                  ...
                  {cert.issued_at ? (
                    <p className="mt-1 text-[11px] font-semibold text-[#0d1b2a]/40">{fmtDate(cert.issued_at)}</p>
                  ) : cert.created_at ? (
                    <p className="mt-1 text-[11px] font-semibold text-[#0d1b2a]/40">{fmtDate(cert.created_at)}</p>
                  ) : null}
          ...
          {lastPage > 1 && (
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
                ...
                onClick={() => load(page - 1)}
                className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-[#0d1b2a] transition hover:bg-slate-50 disabled:opacity-40"
              >
