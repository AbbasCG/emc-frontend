...
function SubmissionCard({
  ...
  return (
    <article className={`flex flex-col gap-4 rounded-2xl border bg-white p-5 shadow-sm transition hover:shadow-md ${
      overdue ? 'border-red-200/80' : needsRev ? 'border-amber-200/80' : 'border-[#0C2A4B]/[0.07]'
    }`}>
      {/* Row 1: status + course */}
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {assignment.course_name && (
          <span className="inline-flex items-center gap-1 rounded-full border border-[#0077B6]/20 bg-[#0077B6]/[0.06] px-2.5 py-1 text-[11px] font-bold text-[#1a6fa0]">
      ...
      <div className="flex items-start gap-2.5">
        <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-[#F28C00]/12 text-[#F28C00]">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {dueLabel !== '—' && (
          <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 ${
            overdue ? 'border-red-200/60 bg-red-50/60' : 'border-[#0C2A4B]/[0.07] bg-slate-50/70'
          }`}>
            <Clock className={`h-3.5 w-3.5 shrink-0 ${overdue ? 'text-red-500' : 'text-[#0C2A4B]/40'}`} />
            <div className="min-w-0">
              <p className="text-[9px] font-black uppercase tracking-wide text-[#0C2A4B]/50">موعد التسليم النهائي</p>
              <p className={`text-[12px] font-black tabular-nums ${overdue ? 'text-red-700' : 'text-[#0C2A4B]'}`}>
                {dueLabel}
              </p>
            </div>
          </div>
...
export default function StudentAssignmentsPage() {
  ...
  return (
    <div className="space-y-6 pb-10 text-right" dir="rtl">
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <header className="flex flex-wrap items-start justify-between gap-4 rounded-3xl border border-[#0C2A4B]/[0.06] bg-gradient-to-bl from-white/95 to-[#0077B6]/[0.03] p-6 shadow-sm ring-1 ring-[#0C2A4B]/[0.04]">
        <div>
          <h1 className="text-2xl font-black text-[#0C2A4B]">واجباتي وتسليماتي</h1>
          <p className="mt-1.5 text-[13px] font-semibold text-[#0C2A4B]/55">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        <StatCard
          ...
          value={stats.total}
          colorClass="border-[#0C2A4B]/[0.07] bg-white text-[#0C2A4B]"
        />
        <StatCard
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[220px]">
          <Search className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#0C2A4B]/30" />
          <input
            type="text"
            dir="rtl"
            ...
            className="w-full rounded-2xl border border-[#0C2A4B]/12 bg-white py-2.5 pr-9 pl-4 text-[13px] font-semibold text-[#0C2A4B] outline-none ring-1 ring-transparent transition focus:border-[#0077B6]/30 focus:ring-[#0077B6]/15"
          />
        </div>

        {/* Status filter */}
        <div className="flex rounded-2xl border border-[#0C2A4B]/12 bg-white p-1 gap-1">
          {[
            { key: 'all',      label: 'الكل' },
            ...
            <button
              ...
              className={`rounded-xl px-3 py-1.5 text-[11px] font-black transition ${
                filterStatus === key
                  ...
                  : 'text-[#0C2A4B]/60 hover:bg-slate-100'
              }`}
            >
              {label}
            </button>
      ...
      {submitted.length === 0 ? (
        <div className="rounded-3xl bg-white/80 ring-1 ring-[#0C2A4B]/[0.06]">
          ...
          <div className="p-6 text-center">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-[#0C2A4B] px-5 py-2.5 text-[12px] font-black text-white transition hover:opacity-90"
            >
              العودة إلى لوحتي
            </Link>
          </div>
        ...
        <div className="rounded-3xl border border-[#0C2A4B]/[0.06] bg-white/80 p-10 text-center">
          <p className="text-[14px] font-black text-[#0C2A4B]/60">لا نتائج تطابق البحث أو التصفية</p>
          <button type="button" onClick={() => { setSearch(''); setFilterStatus('all') }} className="mt-4 text-[12px] font-black text-[#0077B6] hover:underline">
            إعادة تعيين
          </button>
        </div>
      ) : (
