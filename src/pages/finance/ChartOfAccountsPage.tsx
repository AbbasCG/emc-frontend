        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span
            className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-bold ${
              isDebit ? 'bg-emerald-50 text-emerald-700 border border-emerald-200/60' : 'bg-accent-50 text-accent-700 border border-accent-200/60'
            }`}
          >
          ...
          <span
            className={`rounded-md px-2 py-0.5 text-[10px] font-bold ${
              account.is_selectable
                ? 'bg-brand-50 text-customBlue border border-brand-200/60'
...
export default function ChartOfAccountsPage() {
  ...
  // Manual refresh (button / after mutations) = event handler, so the
  // synchronous setLoading(true) here is allowed and required.
  ...
  async function handleCreateSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(apiMessage(err) ?? 'تعذر إضافة الحساب')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(apiMessage(err) ?? 'تعذر تحديث الحساب')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function handleDelete(account: ChartOfAccountItem) {
    ...
    try {
      ...
      toast.error(apiMessage(err) ?? 'تعذر حذف الحساب')
    }
  }
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
        <div className="flex items-center gap-4">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-customBlue to-[#0E5A8A] text-white shadow-md">
            <FolderTree size={24} />
          </div>
          <div>
