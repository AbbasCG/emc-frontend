...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      title="نسخ"
      className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 transition hover:border-[#2691C2]/40 hover:text-[#2691C2]"
    >
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#2691C2]/10">
                <Receipt size={16} className="text-[#2691C2]" />
              </div>
              <div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5">
            ...
            {order.invoice && (
              <button
                type="button"
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#2691C2]/[0.08] py-3 text-[13px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/[0.14]"
              >
                <Download size={14} />
                تحميل الفاتورة
              </button>
...
function KpiCard({ icon: Icon, label, value, accent = 'border-t-[#2691C2]' }: {
...
export default function FinanceOrdersPage() {
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#2691C2]/10">
            <Receipt size={20} className="text-[#2691C2]" />
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <KpiCard icon={BadgeDollarSign} label="مدفوعة"            value={String(paid.length)}              accent="border-t-[#2691C2]"   />
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input
            ...
            className="h-9 w-full rounded-xl border border-slate-200 bg-white pe-9 ps-3 text-[13px] text-slate-700 placeholder:text-slate-400 focus:border-[#2691C2]/40 focus:outline-none focus:ring-2 focus:ring-[#2691C2]/15"
          />
        </div>
        <select
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value)}
          className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-[13px] text-slate-700 focus:border-[#2691C2]/40 focus:outline-none"
        >
      ...
      >
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <Loader2 size={28} className="animate-spin text-[#2691C2]/50" />
          </div>
        ) : filtered.length === 0 ? (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="rounded-xl border border-deepBlue/[0.08] bg-white px-3 py-1.5 text-[11px] font-black text-[#2691C2] shadow-sm transition hover:border-[#2691C2]/30 hover:bg-[#2691C2]/[0.05]"
                        >
                          تفاصيل
                        </button>
                      </td>
