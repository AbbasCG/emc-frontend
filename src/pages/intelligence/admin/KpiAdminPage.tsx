...
const CHART_COLORS = ['#2691C2', '#EC943C', '#22334A', '#10b981', '#6366f1', '#f43f5e']
    ...
    <motion.div
      ...
      className={`rounded-2xl border bg-gradient-to-br p-5 shadow-sm ${
        metric.accent === 'orange'
          ? 'border-[#EC943C]/25 from-[#EC943C]/10 to-white'
          : 'border-[#2691C2]/20 from-[#2691C2]/8 to-white'
      }`}
    >
      <p className="text-[10px] font-black uppercase tracking-wider text-[#22334A]/45">{metric.label}</p>
      <p className="mt-2 text-2xl font-black tabular-nums text-[#22334A]">{metric.value}</p>
      {metric.hint ? <p className="mt-1 text-[10px] font-medium text-[#22334A]/55">{metric.hint}</p> : null}
    </motion.div>
  )
}
...
function EmptyAnalytics({ message }: { message: string }) {
  return (
    <div className="rounded-xl border border-dashed border-[#22334A]/15 bg-[#f8fafc] px-6 py-12 text-center">
      <LineChart size={28} className="mx-auto text-[#22334A]/25" />
      <p className="mt-3 text-sm font-black text-[#22334A]/65">{message}</p>
      <p className="mx-auto mt-2 max-w-md text-[12px] font-medium text-[#22334A]/45">
...
export default function KpiAdminPage() {
  ...
  return (
    <div dir="rtl" className="space-y-6 text-right">
      {/* KPI Hero */}
      <section className="relative overflow-hidden rounded-[1.75rem] border border-[#22334A]/10 bg-gradient-to-bl from-[#22334A] via-[#1a3550] to-[#2691C2] p-8 shadow-xl">
        <div aria-hidden className="pointer-events-none absolute -left-20 top-0 h-56 w-56 rounded-full bg-white/10 blur-[90px]" />
        <div aria-hidden className="pointer-events-none absolute bottom-0 right-[15%] h-40 w-40 rounded-full bg-[#EC943C]/25 blur-[70px]" />
      ...
      {/* Tab filter */}
      <nav className="flex flex-wrap justify-end gap-2 rounded-2xl border border-[#22334A]/8 bg-white/90 p-2 shadow-sm backdrop-blur-sm">
        {tabs.map((t) => (
          <button
            key={t.slug}
            type="button"
            ...
            className={`rounded-xl px-4 py-2 text-xs font-black transition ${
              tab === t.slug ? 'bg-[#22334A] text-white shadow-md' : 'bg-[#f8fafc] text-[#22334A] ring-1 ring-[#22334A]/8 hover:bg-white'
            }`}
          >
            {t.label}
          </button>
      ...
      : loadError ?
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-10 text-center">
          ...
          <button type="button" onClick={() => void load()} className="mt-5 rounded-xl bg-[#22334A] px-6 py-2.5 text-sm font-black text-white">
            إعادة المحاولة
          </button>
        </div>
      : (
        <>
          ...
          {summaryCards.length > 0 ?
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              {summaryCards.map((c) => (
                <div key={c.label} className="rounded-2xl border border-[#22334A]/8 bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] font-black text-[#22334A]/45">{c.label}</p>
                    <c.icon size={16} className="text-[#2691C2]" />
                  </div>
                  <p className="mt-2 text-xl font-black tabular-nums text-[#22334A]">{typeof c.value === 'number' ? fmt(c.value) : c.value}</p>
                </div>
              ))}
            </div>
          : null}
          ...
          {highlights.length > 0 ?
            <ul className="rounded-2xl border border-[#2691C2]/15 bg-gradient-to-bl from-[#2691C2]/8 to-white p-5">
              <p className="text-[10px] font-black uppercase tracking-wider text-[#22334A]/45">ملخص تنفيذي</p>
              <ul className="mt-3 space-y-2">
                {highlights.map((h, i) => (
                  <li key={i} className="text-sm font-semibold text-[#22334A]/80">· {h}</li>
                ))}
              </ul>
            </ul>
          : null}
          ...
          <section className="rounded-2xl border border-white/80 bg-white/95 p-5 shadow-sm">
            <h2 className="mb-4 flex items-center gap-2 text-sm font-black text-[#22334A]">
              <LineChart size={16} className="text-[#2691C2]" /> تحليلات الأداء (بيانات حية)
            </h2>
            ...
            : (
              <div className="grid gap-4 lg:grid-cols-2">
                {registrationChart.some((p) => p.count > 0) ?
                  <div className="rounded-xl border border-[#22334A]/8 bg-[#f8fafc] p-4">
                    <p className="mb-2 text-[11px] font-black text-[#22334A]/55">اتجاه التسجيلات</p>
                    <div className="h-[200px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={registrationChart}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#22334A14" vertical={false} />
                          ...
                          <Area type="monotone" dataKey="count" stroke="#2691C2" fill="#2691C2" fillOpacity={0.15} strokeWidth={2} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                {coursesChart.some((p) => p.count > 0) ?
                  <div className="rounded-xl border border-[#22334A]/8 bg-[#f8fafc] p-4">
                    <p className="mb-2 text-[11px] font-black text-[#22334A]/55">نمو الدورات</p>
                    <div className="h-[200px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={coursesChart}>
                          ...
                          <YAxis allowDecimals={false} tick={{ fontSize: 10 }} width={28} />
                          <Tooltip formatter={(v) => tooltipCount(v)} />
                          <Bar dataKey="count" fill="#EC943C" radius={[6, 6, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border border-white/80 bg-white/95 p-5 shadow-sm">
              <h2 className="mb-4 text-sm font-black text-[#22334A]">تحليل التسجيلات</h2>
            ...
            {/* Assignments */}
            <section className="rounded-2xl border border-white/80 bg-white/95 p-5 shadow-sm">
              <h2 className="mb-4 text-sm font-black text-[#22334A]">تحليل الواجبات</h2>
          ...
          {/* Export center */}
          <section className="rounded-2xl border border-[#22334A]/8 bg-gradient-to-br from-[#f8fafc] to-white p-6">
            <h2 className="text-sm font-black text-[#22334A]">مركز التصدير</h2>
            <p className="mt-2 text-[12px] font-medium text-[#22334A]/55">
              يمكنك إنشاء تقارير PDF/Excel مفصّلة من لوحة التقارير — البيانات تُسحب من نفس مصادر المؤشرات أعلاه.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              <Link to="/dashboard/admin/reports" className="rounded-xl bg-[#22334A] px-4 py-2.5 text-[11px] font-black text-white hover:bg-[#1a2a3d]">
                فتح مركز التقارير
              </Link>
              {isProgramsManager ?
                <Link to="/dashboard/programs-manager" className="rounded-xl border border-[#22334A]/15 bg-white px-4 py-2.5 text-[11px] font-black text-[#22334A] hover:bg-[#f8fafc]">
                  لوحة مدير البرامج
                </Link>
              : null}
            </div>
