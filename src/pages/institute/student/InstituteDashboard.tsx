...
interface InstitutePricingOption {
  title: string;
  price: number;
}

interface InstituteLevel {
  id: number;
  title: string;
...
/** Shape of GET /institute/student/dashboard, discriminated by enrollment status. */
type InstituteStudentDashboardData =
  | { status: 'not_enrolled'; available_levels?: InstituteLevel[] }
  | { status: 'waitlist'; current_level: string }
  | { status: 'enrolled'; current_level: string; progress: number; next_class: InstituteNextClass };

export default function InstituteDashboard() {
  const [dashboardData, setDashboardData] = useState<InstituteStudentDashboardData | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setDashboardData(res.data as InstituteStudentDashboardData);
      } catch (err) {
        if (controller.signal.aborted) return;
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {dashboardData.available_levels?.map((level) => (
                    ...
                    <div className="mt-6 border-t pt-4">
                      {level.pricing_options ? (
                        <div className="space-y-2 mb-4">
                          ...
                          {level.pricing_options.map((opt, i) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { title: 'Student Book PDF', icon: BookOpen, color: 'text-brand-700', bg: 'bg-brand-50' },
                      { title: 'Audio Tracks', icon: Headphones, color: 'text-teal-700', bg: 'bg-teal-50' },
