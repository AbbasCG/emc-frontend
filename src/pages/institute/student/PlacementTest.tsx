...
interface PlacementQuestion {
  id: number;
  type: 'reading' | 'grammar' | 'listening';
  question: string;
  text?: string;
  audioSrc?: string;
  options: string[];
...
export default function PlacementTest() {
  ...
  // Timer logic — ticking state is set inside the interval callback (effect-patterns.md P3).
  useEffect(() => {
  ...
  // Mock API call to submit answers and get level — setLevel runs in the timeout callback.
  useEffect(() => {
  ...
  // Time ran out → finish the test (render-phase state adjustment, effect-patterns.md P2).
  if (started && !isFinished && timeLeft === 0) {
    setIsFinished(true);
  }
