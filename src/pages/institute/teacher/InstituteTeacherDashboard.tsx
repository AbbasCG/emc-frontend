...
export default function InstituteTeacherDashboard() {
  ...
  const [activeClassGroup, setActiveClassGroup] = useState<any>(null);
  ...
  const handleManageClass = async (classId: number) => {
    ...
    try {
      ...
      setActiveClassGroup(res.data.class_group);
      // Initialize status for attendance UI
      const studentsWithStatus = res.data.students.map((s:any) => ({...s, status: 'pending'}));
