...
export default function WorkshopDetailsPage() {
  ...
  function renderCta(size: 'sm' | 'lg' = 'lg') {
    ...
    if (externalUrl) {
      return (
        <a href={externalUrl} target="_blank" rel="noopener noreferrer" className={`${base} bg-[#F28C00]`}>
    ...
    if (!isAuthenticated) {
      return (
        <Link to={buildPublicLoginHref(redirectPath)} className={`${base} bg-[#F28C00]`}>
    ...
    if (!isStudent) {
      return (
        <button type="button" onClick={() => toast.error(PUBLIC_ENROLL_STUDENT_ONLY_MSG)} className={`${base} bg-[#F28C00]`}>
          <GraduationCap size={18} />
          سجّل في الورشة
        </button>
      )
    ...
    if (workshop?.course_slug) {
      return (
        <Link to={buildWorkshopDetailEnrollHref(workshop.course_slug)} className={`${base} bg-[#F28C00]`}>
          <GraduationCap size={18} />
          سجّل في الورشة
        </Link>
      )
    ...
    return (
      <button type="button" onClick={scrollToEnroll} className={`${base} bg-[#F28C00]`}>
        <GraduationCap size={18} />
        سجّل في الورشة
      </button>
    )
  ...
  if (loading) {
    return (
      <main className={`relative overflow-x-hidden bg-gradient-to-br from-[#0C2A4B] to-[#0077B6] ${PAGE_TOP}`} dir="rtl">
  ...
  const enrollSidebar = (
    <div className="overflow-hidden text-right">
      ...
      <div className="border-b border-[#0C2A4B]/6 px-5 py-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="text-left">
            <span className={`text-xl font-black tabular-nums ${isFree ? 'text-[#0077B6]' : 'text-[#F28C00]'}`}>
      ...
      {w.instructor_name && (
        <div className="border-t border-[#0C2A4B]/6 bg-gradient-to-br from-[#0077B6]/6 to-white px-4 py-3.5 text-right">
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-black text-[#0C2A4B]">{w.instructor_name}</p>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <main className={`relative overflow-x-hidden bg-[#0C2A4B] pb-20 ${PAGE_TOP} lg:pb-8`} dir="rtl">
      <PublicSeo
        title={w.title}
        description={seoDesc}
        ...
        extra={
          !isAuthenticated ? (
            <Link
              ...
              className="inline-flex h-12 flex-1 items-center justify-center rounded-xl bg-[#F28C00] px-4 text-sm font-black text-white"
            >
              تسجيل الدخول للالتحاق
            </Link>
          ) : externalUrl ? (
