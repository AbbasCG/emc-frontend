...
export default function LearningPathDetail() {
  ...
  return (
    <main className="bg-slate-50 pt-20" dir="rtl">
      ...
      />
      {/* ── HERO ─────────────────────────────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-gradient-to-bl from-[#22334A] via-[#1c4567] to-[#162334] py-20 text-white">
        {path.featured_image && (
          <img
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1fr_380px]">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="visible" transition={{ duration: 0.5 }}>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                {path.is_featured && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#EC943C] px-3 py-1 text-xs font-black">
              ...
              <div className="flex flex-wrap gap-6 text-sm">
                {durationLabel && (
                  <div className="flex items-center gap-2">
                    <Clock className="h-5 w-5 text-[#2691C2]" />
                    <span>{durationLabel}</span>
                  </div>
                )}
                <div className="flex items-center gap-2">
                  <BookOpen className="h-5 w-5 text-[#EC943C]" />
            ...
            <motion.div
              ...
              className="rounded-3xl bg-white p-8 text-[#22334A] shadow-2xl"
            >
              {path.featured_image ? (
                <img
                  src={path.featured_image}
                ...
                <div className="mb-6 flex h-44 w-full items-center justify-center rounded-2xl bg-gradient-to-br from-[#2691C2] to-[#22334A]">
              ...
              <div className="mb-2">
                ...
                <p className="text-3xl font-black text-[#22334A]">
              ...
              {enrollStatus.enrolled ? (
                ...
                <button
                  ...
                  className="block w-full rounded-2xl bg-[#2691C2] py-3.5 text-center font-black text-white shadow-lg shadow-[#2691C2]/25 transition hover:bg-[#1d7aab] disabled:opacity-60"
                >
                  {enrolling ? (
              ...
              <p className="mt-3 text-center text-xs text-slate-400">
                <Link to="/contact" className="text-[#2691C2] hover:underline">
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_380px]">
          <div className="space-y-12">
            ...
            {(path.courses ?? []).length > 0 && (
              <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                <h2 className="mb-6 text-2xl font-black text-[#22334A]">رحلة التعلم</h2>
                <div className="space-y-4">
                  {(path.courses ?? []).map((course, i) => (
                    ...
                    >
                      {/* Step indicator */}
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#2691C2] text-sm font-black text-white shadow-md shadow-[#2691C2]/30">
                        {i + 1}
                      </div>
                      {/* Course info */}
                      <div className="flex-1 text-right">
                        <div className="mb-1 flex flex-wrap items-center justify-start gap-2">
                          ...
                          <h3 className="font-black text-[#22334A]">{course.title}</h3>
                        </div>
            ...
            {path.learning_outcomes.length > 0 && (
              <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                <h2 className="mb-5 text-2xl font-black text-[#22334A]">ماذا ستتعلم</h2>
                <div className="grid gap-3 sm:grid-cols-2">
                  {path.learning_outcomes.map((item, i) => (
                    <div key={i} className="flex items-start gap-3 rounded-xl bg-[#2691C2]/5 p-4">
                      <CheckCircle className="mt-0.5 h-5 w-5 shrink-0 text-[#2691C2]" />
            ...
            {path.requirements.length > 0 && (
              <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                <h2 className="mb-5 text-2xl font-black text-[#22334A]">المتطلبات</h2>
                <ul className="space-y-2">
                  {path.requirements.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-slate-600">
                      <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[#EC943C]" />
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    <h2 className="text-xl font-black text-[#22334A]">شهادة الإتمام</h2>
            ...
            {path.instructor && (
              <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                <h2 className="mb-5 text-2xl font-black text-[#22334A]">المدرب</h2>
                <div className="flex items-center gap-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  {path.instructor.avatar_url ? (
                    ...
                    <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#2691C2]/10 text-xl font-black text-[#2691C2]">
                      {path.instructor.name.charAt(0)}
                    </div>
                  )}
                  <div className="text-right">
                    <h3 className="text-lg font-black text-[#22334A]">{path.instructor.name}</h3>
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-28 rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
              <div className="mb-4 text-right">
                ...
                <p className="text-4xl font-black text-[#22334A]">
                  {effectivePrice === 0 || effectivePrice == null
                    ? 'مجاناً'
                    : formatEuroInteger(effectivePrice, 'ar')}
                </p>
              ...
              <Link
                to="/contact"
                className="mb-3 block w-full rounded-2xl bg-[#2691C2] py-4 text-center font-black text-white shadow-lg shadow-[#2691C2]/25 transition hover:bg-[#1d7aab]"
              >
                سجّل في المسار الآن
              </Link>
              <Link
                to="/contact"
                className="block w-full rounded-2xl border border-slate-200 py-3.5 text-center text-sm font-semibold text-slate-600 transition hover:border-[#2691C2] hover:text-[#2691C2]"
              >
                استفسر عن المسار
              </Link>
              ...
              <ul className="mt-6 space-y-3 text-right text-sm text-slate-600">
                {durationLabel && (
                  <li className="flex items-center justify-between">
                    ...
                    <Clock className="h-4 w-4 text-[#2691C2]" />
                  </li>
                )}
                <li className="flex items-center justify-between">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200 bg-white p-4 shadow-lg lg:hidden">
        <div className="flex items-center justify-between gap-4">
          <p className="text-xl font-black text-[#22334A]">
            {effectivePrice === 0 || effectivePrice == null
              ? 'مجاناً'
          ...
          <Link
            to="/contact"
            className="flex-1 rounded-2xl bg-[#2691C2] py-3 text-center font-black text-white transition hover:bg-[#1d7aab]"
          >
            سجّل الآن
          </Link>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:pb-16 lg:px-8">
        <Link
          to="/learning-paths"
          className="inline-flex items-center gap-2 text-sm font-semibold text-[#2691C2] hover:underline"
        >
