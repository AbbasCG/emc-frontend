...
function JsonDiff({ label, data, colorClass }: { label: string; data: any; colorClass: string }) {
    ...
    <div dir="rtl" className="min-h-screen bg-slate-50 p-6">
      ...
      {meta.last_page > 1 && (
        <div className="flex items-center justify-center gap-2 mt-6">
          ...
          <button onClick={() => setPage(p => Math.min(meta.last_page, p + 1))} disabled={page === meta.last_page} className="p-2 rounded-xl border border-slate-200 disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button>
        </div>
      )}
