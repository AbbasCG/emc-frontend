...
import { Calendar, Plus, Users, LayoutDashboard, Flag } from 'lucide-react'
...
export default function MeetingLoungePage() {
  ...
  const [defaultMeetingType, setDefaultMeetingType] = useState<'department' | 'executive' | 'general'>('department')
  ...
  const handleOpenCreate = (type: 'department' | 'executive' | 'general') => {
    ...
    <div dir="rtl" className="space-y-8 text-right">
      <header className="rounded-2xl border border-deepBlue/[0.06] bg-white p-6 shadow-sm ring-1 ring-deepBlue/[0.03]">
        <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">
              {user?.department?.name ? `إدارة ${user.department.name}` : 'إدارتك الحالية'} — جدولة وإدارة تقارير الاجتماعات
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <button 
            onClick={() => handleOpenCreate('executive')}
