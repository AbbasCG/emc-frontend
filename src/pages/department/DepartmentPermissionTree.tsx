...
export default function DepartmentPermissionTree() {
  ...
  const fetchTree = async (employee: Employee) => {
    ...
    try {
      const response = await apiClient.get(`/department/employees/${employee.id}/permission-tree`);
      if (response.data.success) {
  ...
  const togglePermission = async (node: PermissionNode) => {
    ...
    try {
      await apiClient.post(`/department/employees/${selectedEmployee.id}/permissions/${action}`, {
