...
function ModuleModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-[#0C2A4B]/[0.08]"
        dir="rtl"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#0C2A4B]/[0.06] px-6 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-[#0C2A4B]/40 hover:bg-slate-100"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          <div className="space-y-4 px-6 py-5">
            ...
            {/* Sort order */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">ترتيب العرض</label>
              <input
                type="number"
                min={0}
        ...
        <div className="flex justify-end gap-2 border-t border-[#0C2A4B]/[0.05] px-6 py-4">
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-[#0C2A4B]/10 px-5 py-2 text-[12px] font-black text-[#0C2A4B] hover:bg-slate-50"
          >
            إلغاء
          </button>
          <button
...
function ModuleCard({
  ...
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="group flex flex-col rounded-2xl border border-[#0C2A4B]/6 bg-white shadow-sm transition hover:border-[#0077B6]/20 hover:shadow-md"
    >
      {/* Card header */}
      <div className="flex items-start gap-3 p-4 pb-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#0077B6]/10 ring-1 ring-[#0C2A4B]/6">
          <Layers size={17} className="text-[#0077B6]" aria-hidden />
        </span>
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className="line-clamp-2 text-[13px] font-bold leading-snug text-[#0C2A4B]">{m.title ?? '—'}</p>
            {statusChip(m.status)}
          </div>
          {courseLabel && (
            <div className="mt-1 flex items-center gap-1">
              <GraduationCap size={11} className="shrink-0 text-[#0077B6]" aria-hidden />
              <p className="line-clamp-1 text-[11px] font-bold text-[#0077B6]">{courseLabel}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Description */}
      {m.description && (
        <p className="px-4 pb-2 text-[11px] leading-relaxed text-[#0C2A4B]/45 line-clamp-2">{m.description}</p>
      )}
      ...
      {/* Footer */}
      <div className="flex items-center justify-between gap-2 border-t border-[#0C2A4B]/5 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <p className="text-[10px] font-medium text-[#0C2A4B]/30">{fmtDate(m.created_at ?? m.updated_at)}</p>
          {m.sort_order != null && (
...
function ModuleDrawer({
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Drawer header */}
        <div className="flex items-start justify-between gap-3 border-b border-[#0C2A4B]/8 px-5 py-4">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-[#0077B6]">وحدة تعليمية</p>
            <h2 className="mt-0.5 line-clamp-2 text-[15px] font-black text-[#0C2A4B]">{detail.title}</h2>
            {courseLabel && (
              <div className="mt-1 flex items-center gap-1">
                <GraduationCap size={11} className="text-[#0077B6]" />
                <span className="text-[11px] font-bold text-[#0077B6]">{courseLabel}</span>
              </div>
            )}
          </div>
          <div className="flex shrink-0 items-center gap-1">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-[#0C2A4B]/10 px-3 py-1.5 text-[11px] font-black text-[#0C2A4B] transition hover:bg-slate-50"
            >
        ...
        {/* Tabs */}
        <div className="flex gap-0 overflow-x-auto border-b border-[#0C2A4B]/8 px-4">
            ...
            <button
              ...
              className={[
                ...
                tab === key
                  ...
                  : 'border-transparent text-[#0C2A4B]/45 hover:text-[#0C2A4B]',
              ].join(' ')}
            >
              <Icon size={12} />
              {label}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={label} className="rounded-xl border border-[#0C2A4B]/6 bg-slate-50 p-3">
                    <p className="text-[10px] font-black text-[#0C2A4B]/40">{label}</p>
                    <p className="mt-0.5 text-[13px] font-bold text-[#0C2A4B]">{value}</p>
                  </div>
                ))}
              </div>
              {detail.description && (
                <div className="rounded-xl border border-[#0C2A4B]/6 bg-slate-50 p-4">
                  <p className="mb-1 text-[10px] font-black text-[#0C2A4B]/40">الوصف</p>
                  <p className="text-[13px] leading-relaxed text-[#0C2A4B]/70">{detail.description}</p>
                </div>
              )}
            ...
            <DrawerList
              ...
              renderItem={(l: { id: number; title?: string; sort_order?: number; status?: string }) => (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[12px] font-bold text-[#0C2A4B]">{l.title ?? `درس ${l.id}`}</p>
                    {l.sort_order != null && <p className="text-[10px] text-[#0C2A4B]/40">ترتيب {l.sort_order}</p>}
                  </div>
            ...
            <DrawerList
              ...
              renderItem={(s: { id: number; title?: string; session_date?: string; status?: string }) => (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[12px] font-bold text-[#0C2A4B]">{s.title ?? `جلسة ${s.id}`}</p>
                    {s.session_date && <p className="text-[10px] text-[#0C2A4B]/40">{fmtDate(s.session_date)}</p>}
                  </div>
            ...
            <DrawerList
              ...
              renderItem={(mat: { id: number; title?: string; type?: string }) => (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[12px] font-bold text-[#0C2A4B]">{mat.title ?? `مادة ${mat.id}`}</p>
                    {mat.type && <p className="text-[10px] text-[#0C2A4B]/40">{mat.type}</p>}
                  </div>
                </div>
              )}
            />
            ...
            <DrawerList
              ...
              renderItem={(a: { id: number; title?: string; due_date?: string }) => (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[12px] font-bold text-[#0C2A4B]">{a.title ?? `واجب ${a.id}`}</p>
                    {a.due_date && <p className="text-[10px] text-[#0C2A4B]/40">الموعد النهائي: {fmtDate(a.due_date)}</p>}
                  </div>
                </div>
              )}
            />
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center">
        <p className="text-[13px] font-semibold text-[#0C2A4B]/35">{emptyText}</p>
      </div>
    )
  }
  return (
    <div className="space-y-2">
      <p className="mb-3 text-[11px] font-black text-[#0C2A4B]/40">{items.length} {label}</p>
      {items.map((item, i) => (
        <div key={i} className="rounded-xl border border-[#0C2A4B]/6 bg-white p-3 hover:bg-slate-50">
          {renderItem(item)}
        </div>
      ))}
    </div>
...
function StatCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="rounded-2xl border border-[#0C2A4B]/6 bg-white p-5 shadow-sm">
      <p className="text-[11px] font-black uppercase tracking-wider text-[#0C2A4B]/40">{label}</p>
      <p className="mt-1 text-2xl font-black text-[#0C2A4B]">{value}</p>
      {sub && <p className="mt-0.5 text-[11px] font-semibold text-[#0C2A4B]/30">{sub}</p>}
    </div>
  )
}
...
export function AdminModulesPage() {
  ...
  return (
    <AdminLmsShell title="إدارة الوحدات" description="مصدر البيانات الموحد مع صفحة محتوى الدورة" breadcrumb="الوحدات">
      <div dir="rtl" className="space-y-6 text-right">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={load}
              disabled={loading}
              className="inline-flex items-center gap-2 rounded-xl border border-[#0C2A4B]/10 bg-white px-4 py-2.5 text-[12px] font-bold text-[#0C2A4B] shadow-sm transition hover:bg-slate-50 disabled:opacity-50"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-[#0077B6] px-4 py-2.5 text-[12px] font-black text-white shadow-sm transition hover:bg-[#0077B6]/90"
            >
        ...
        {/* Filter bar */}
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-[#0C2A4B]/6 bg-white p-4 shadow-sm">
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="بحث في الوحدات..."
            className="min-w-[180px] flex-1 rounded-xl border border-[#0C2A4B]/10 bg-[#0C2A4B]/[0.02] px-3 py-2 text-sm font-semibold text-[#0C2A4B] outline-none focus:border-[#0077B6]/40 focus:ring-2 focus:ring-[#0077B6]/10"
          />
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="rounded-xl border border-[#0C2A4B]/10 bg-white px-3 py-2 text-[12px] font-black text-[#0C2A4B] outline-none focus:border-[#0077B6]/40"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl border border-[#0C2A4B]/10 bg-white px-3 py-2 text-[12px] font-black text-[#0C2A4B] transition hover:bg-slate-50"
        ...
        {loading && !error && (
          <div className="flex min-h-[16rem] items-center justify-center rounded-2xl border border-[#0C2A4B]/6 bg-white">
            <Loader2 className="h-10 w-10 animate-spin text-[#0077B6]" aria-hidden />
          </div>
        )}
 ...
 {rows.length === 0 ? 'لا توجد وحدات أنشئ وحدتك الأولى الآن.': 'لا نتائج تطابق الفلاتر الحالية.'}
        ...
        {!loading && !error && rows.length > 0 && (
          <p className="text-[11px] font-semibold text-[#0C2A4B]/35">
...
const lfieldCls = 'w-full rounded-xl border border-[#0C2A4B]/10 bg-[#0C2A4B]/[0.02] px-3 py-2.5 text-sm font-semibold text-[#0C2A4B] outline-none focus:border-[#0077B6]/40 focus:ring-2 focus:ring-[#0077B6]/10'
      ...
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-[#0C2A4B]/[0.08]" dir="rtl">
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          <div className="space-y-4 px-6 py-5">
            {/* Module */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">الوحدة <span className="text-rose-500">*</span></label>
              <select
            ...
            {/* Title */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">عنوان الدرس <span className="text-rose-500">*</span></label>
              <input
                value={form.title}
                onChange={(e) => set('title', e.target.value)}
            ...
            {/* Video URL */}
            <div>
              <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">رابط الفيديو</label>
              <input
                type="url"
                dir="ltr"
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">المدة (دقيقة)</label>
                <input
                  type="number"
                  min="0"
              ...
              <div>
                <label className="mb-1.5 block text-[12px] font-black text-[#0C2A4B]/60">الترتيب</label>
                <input
                  type="number"
                  min="0"
                  value={form.sort_order}
...
function LessonCard({
  ...
  return (
    ...
    >
      <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${hasVideo ? 'bg-violet-50' : 'bg-[#0077B6]/8'} ring-1 ring-[#0C2A4B]/6`}>
        {hasVideo
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <p className="line-clamp-1 text-sm font-bold text-[#0C2A4B]">{row.title ?? '—'}</p>
        ...
        {(row.module_title || row.course_title || row.course_name) && (
          <p className="mt-0.5 line-clamp-1 text-xs font-semibold text-[#0077B6]/80">
            {row.module_title ?? '—'}
            {(row.course_title ?? row.course_name) ? ` · ${normCourseTitle(row.course_title ?? row.course_name)}` : ''}
          </p>
        )}
        {row.description && (
          <p className="mt-1 line-clamp-2 text-xs text-[#0C2A4B]/40">{row.description}</p>
        )}
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <p className="text-[11px] font-medium text-[#0C2A4B]/35">{fmtDate(row.created_at ?? row.updated_at)}</p>
            {row.duration_minutes ? (
              <span className="flex items-center gap-1 text-[11px] font-medium text-[#0C2A4B]/35">
                <Clock size={10} />
                {row.duration_minutes} د
              </span>
            ) : null}
            {row.sort_order != null && (
              <span className="text-[11px] font-medium text-[#0C2A4B]/25">#{row.sort_order}</span>
            )}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[220px] flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white py-2 pr-9 pl-3 text-[12px] font-semibold text-slate-700 shadow-sm focus:border-[#0077B6] focus:outline-none"
            />
          </div>
          <button
            type="button"
            onClick={() => setShowFilters((v) => !v)}
            className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[12px] font-black shadow-sm transition ${showFilters ? 'border-[#0077B6]/30 bg-[#0077B6]/5 text-[#0077B6]' : 'border-slate-200 bg-white text-slate-700 hover:border-[#0077B6]/20'}`}
          >
            <Filter size={12} />
            فلاتر
          </button>
          <button type="button" onClick={load} className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[12px] font-black text-slate-500 shadow-sm hover:text-[#0077B6]">
            <RefreshCw size={12} />
          </button>
        </div>
        {showFilters && (
        ...
        <div className="py-16 text-center">
          ...
          {rows.length === 0 && (
            <button
              type="button"
              onClick={() => setModal('create')}
              className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#0077B6] px-5 py-2.5 text-[12px] font-black text-white shadow hover:bg-[#0077B6]/90"
            >
