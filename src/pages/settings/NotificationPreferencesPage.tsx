    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[14px] font-black text-[#0C2A4B]">ساعات الهدوء</h3>
          </div>
          <button
            type="button"
            role="switch"
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="text-[14px] font-black text-[#0C2A4B]">إشعارات الدفع (Push)</h3>
        </div>
        {!isPushSupported() ? (
...
function ChannelSummaryCard({ icon: Icon, label, count, color }: { icon: ElementType; label: string; count: number; color: string }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white px-4 py-3 shadow-[0_1px_3px_0_rgba(15,23,42,0.05)]">
      ...
      <div>
        <p className="font-latin text-[18px] font-black tabular-nums text-[#0C2A4B]">{formatEnglishCount(count)}</p>
    ...
    >
      <div className="border-b border-slate-100 bg-gradient-to-bl from-slate-50/80 to-white px-5 py-4">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#0C2A4B]/[0.08]">
            <Icon className="h-5 w-5 text-[#0C2A4B]" />
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h2 className="text-[15px] font-black text-[#0C2A4B]">{category.labelAr}</h2>
...
export default function NotificationPreferencesPage() {
  ...
  return (
    <div dir="rtl" className="pb-28 text-[#0C2A4B]">
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
        ...
        >
          <div>
            <h1 className="text-xl font-black tracking-tight text-[#0C2A4B] sm:text-[22px]">تفضيلات الإشعارات</h1>
        ...
        {!loading && (
          ...
          >
            <ChannelSummaryCard icon={Bell} label="إجمالي الأحداث" count={total} color="#0C2A4B" />
            <ChannelSummaryCard icon={Bell} label="داخل المنصة مفعّل" count={enabledInApp} color="#0077B6" />
            ...
            <div className="flex w-full max-w-6xl flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white px-5 py-4 shadow-[0_8px_32px_rgba(15,23,42,0.12)]">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => void onSave()}
                  disabled={saving}
                  className="rounded-xl bg-[#0077B6] px-5 py-2 text-[12px] font-black text-white shadow-md shadow-[#0077B6]/25 transition hover:bg-[#1f7da8] disabled:opacity-60"
                >
