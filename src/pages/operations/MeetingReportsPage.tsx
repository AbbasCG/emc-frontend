...
export default function MeetingReportsPage() {
  ...
  const [attendance, setAttendance] = useState<Record<number, { attended: boolean; rating: number | null }>>({})
  ...
  async function pickDepartment(id: number | '') {
    ...
    try {
      ...
      setMembers(rows)
      setAttendance(Object.fromEntries(rows.map((m) => [m.id, { attended: true, rating: null }])))
    } catch {
    ...
    try {
      await createMeetingReport({
        ...
        ratings: members.map((m) => ({
          ...
          rating: attendance[m.id]?.attended ? (attendance[m.id]?.rating ?? null) : null,
        })),
      })
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-4">
              ...
              {membersLoading ? (
                ...
                <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                  {members.map((m) => {
                    const row = attendance[m.id] ?? { attended: true, rating: null }
                    return (
                      <li key={m.id} className="flex items-center justify-between gap-3 rounded-lg bg-white px-3 py-2">
                        ...
                        {row.attended && (
                          <RatingPicker
                            value={row.rating}
                            onChange={(v) => setAttendance((a) => ({ ...a, [m.id]: { ...row, rating: v } }))}
                          />
                        )}
                      </li>
                    )
