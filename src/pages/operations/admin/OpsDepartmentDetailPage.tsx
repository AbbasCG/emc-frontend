...
const CARD =
  'rounded-[14px] border border-[#E8EEF4] bg-white shadow-[0_8px_24px_-16px_rgba(34,51,74,0.28)]'
...
function InfoRow({
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 text-[13px] font-bold text-[#22334A] [overflow-wrap:anywhere]" dir="auto">
...
function CardShell({
  ...
  return (
    <section className={`${CARD} p-6 ${className}`}>
      <h2 className="mb-5 flex items-center gap-2.5 text-[14px] font-black text-[#22334A]">
...
function KpiCard({
  ...
  return (
    <div
      className={`${CARD} flex h-full flex-col items-center px-4 py-5 text-center transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_28px_-18px_rgba(34,51,74,0.35)]`}
    >
      ...
      <p className="mt-2 text-[28px] font-black leading-none tabular-nums text-[#22334A]">{value}</p>
      {linkTo && linkLabel ? (
        <Link
          to={linkTo}
          className="mt-3 inline-flex items-center gap-0.5 text-[11px] font-black text-[#2691C2] transition hover:text-[#1f7aa8]"
        >
          {linkLabel}
...
function ActivityMiniCard({
  ...
  return (
    <section className={`${CARD} flex h-full flex-col p-6`}>
      <h3 className="mb-5 flex items-center gap-2.5 text-[13px] font-black text-[#22334A]">
      ...
      <div className="flex flex-1 flex-col items-center justify-center py-2 text-center">
        <p className="text-[32px] font-black tabular-nums leading-none text-[#22334A]">{count}</p>
        <p className="mt-2 text-[12px] font-semibold text-slate-400">{emptyText}</p>
        <Link
          to={linkTo}
          className="mt-4 inline-flex items-center gap-1 text-[12px] font-black text-[#2691C2] hover:underline"
        >
          {linkLabel}
...
function MemberAvatar({ member }: { member: DepartmentOverviewMember }) {
  ...
  return (
    <span
      className="grid h-10 w-10 place-items-center rounded-full bg-[#E8F4FA] text-[11px] font-black text-[#2691C2] ring-2 ring-white"
      title={member.name}
    >
...
export default function OpsDepartmentDetailPage() {
  ...
  if (!overview) {
    return (
      <div dir="rtl" className={`${CARD} p-10 text-center font-black text-[#22334A]`}>
  ...
  return (
    <div dir="rtl" className={`mx-auto flex max-w-[1280px] flex-col ${PAGE_GAP}`}>
      {/* Breadcrumb */}
      <nav className="flex flex-wrap items-center gap-1.5 text-[12px] font-bold text-[#2691C2]" aria-label="مسار التنقل">
        <Link to="/dashboard/admin/departments" className="transition hover:text-[#22334A]">
          الإدارات
        </Link>
      ...
      <motion.header
        ...
        className="relative overflow-hidden rounded-[18px] bg-[#22334A] px-6 py-7 text-white shadow-[0_20px_40px_-24px_rgba(34,51,74,0.55)] sm:px-8"
      >
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(38,145,194,0.22),transparent_45%)]" aria-hidden />
      ...
      <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 xl:grid-cols-6">
        ...
        <KpiCard
          ...
          accent="bg-orange-50 text-[#EC943C]"
      ...
      <div className="grid grid-cols-1 items-start gap-6 xl:grid-cols-2">
        ...
        <div className="flex flex-col gap-6">
          ...
          <CardShell title="القيادة" icon={Users}>
            {manager ? (
              <div className="rounded-[14px] border border-[#E8EEF4] bg-[#F7FAFC] p-5">
                <div className="flex flex-col items-center text-center">
                  <div className="relative">
                    <span className="grid h-[72px] w-[72px] place-items-center overflow-hidden rounded-full bg-[#E8F4FA] text-lg font-black text-[#2691C2] ring-4 ring-white">
                  ...
                  <p className="mt-5 text-[16px] font-black text-[#22334A]">{manager.name}</p>
                ...
                <div className="mt-5 grid grid-cols-1 gap-3 border-t border-[#E8EEF4] pt-4 sm:grid-cols-2">
                  <div className="flex items-center justify-center gap-2 text-[12px] font-bold text-[#22334A]">
                    <Phone className="h-3.5 w-3.5 text-[#2691C2]" aria-hidden />
                    <span dir="ltr">{manager.phone || 'غير متوفر'}</span>
                  </div>
                  <div className="flex items-center justify-center gap-2 text-[12px] font-bold text-[#22334A]">
                    <Mail className="h-3.5 w-3.5 text-[#2691C2]" aria-hidden />
              ...
              <div className="flex flex-col items-center py-6 text-center">
                ...
                {canManage ? (
                  <Link
                    to={editHref}
                    className="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-[#2691C2]/10 px-4 py-2 text-[12px] font-black text-[#2691C2] hover:bg-[#2691C2]/15"
                  >
                    تعيين مدير
                  </Link>
                ) : null}
            ...
            {members.length > 0 ? (
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[18px] font-black text-[#22334A]">
                    {kpis.total_members}{' '}
                ...
                <div className="flex flex-col items-start gap-3 sm:items-end">
                  ...
                  <Link
                    to={membersHref}
                    className="inline-flex items-center gap-1 rounded-xl border border-[#2691C2]/30 bg-[#2691C2]/5 px-3.5 py-2 text-[12px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/10"
                  >
                    عرض جميع الأعضاء
                    <ChevronLeft className="h-3.5 w-3.5" aria-hidden />
                  </Link>
              ...
              <div className="flex flex-col items-center py-6 text-center">
                ...
                <Link
                  to={membersHref}
                  className="mt-4 text-[12px] font-black text-[#2691C2] hover:underline"
                >
                  عرض جميع الأعضاء
                </Link>
              </div>
            ...
            <ul className="divide-y divide-[#E8EEF4]">
              {quickStats.map((row) => (
                <li key={row.label} className="flex items-center justify-between gap-4 py-3.5">
                  ...
                  <span className="text-[15px] font-black tabular-nums text-[#22334A]">{row.value}</span>
                </li>
              ))}
            </ul>
          </CardShell>
                ...
                manager?.phone ? (
                  <span className="inline-flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="rounded-md p-1 text-slate-400 transition hover:bg-slate-100 hover:text-[#2691C2]"
      ...
      <section className={`${CARD} grid grid-cols-1 gap-6 p-6 sm:grid-cols-3`}>
        <div className="text-center sm:text-start">
          ...
          <p className="mt-1.5 text-[13px] font-black tabular-nums text-[#22334A]">
      ...
      {canManage ? (
        <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
            <Link
              to={editHref}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#2F9B8A] px-5 py-3 text-[13px] font-black text-white transition hover:bg-[#278a7a]"
            >
            ...
            {manager?.email ? (
              <a
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#2691C2]/40 bg-white px-5 py-3 text-[13px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/5"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#2691C2]/40 bg-white px-5 py-3 text-[13px] font-black text-[#2691C2]"
              >
                <Send className="h-4 w-4" aria-hidden />
                إرسال رسالة
              </button>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-[13px] font-black text-[#22334A] transition hover:bg-slate-50 disabled:opacity-60"
            >
