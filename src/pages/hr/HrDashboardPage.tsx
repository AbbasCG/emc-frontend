  ...
  const inner = (
    ...
    >
      ...
      <div className="text-right">
        ...
        <p className="mt-0.5 text-3xl font-black text-[#22334A]">{value}</p>
...
function QuickAction({ label, href, icon: Icon, accent }: { label: string; href: string; icon: React.ElementType; accent: string }) {
  return (
    <Link
      to={href}
      className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#22334A] shadow-sm transition hover:border-[#2691C2]/40 hover:bg-sky-50/50 hover:shadow-md"
    >
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-8 shadow-sm">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-[11px] font-black uppercase tracking-widest text-[#2691C2]">EMC — الموارد البشرية</p>
              <h1 className="mt-1 text-2xl font-black text-[#22334A]">قطاع الموارد البشرية</h1>
      ...
      <div className="mx-auto max-w-7xl px-6 py-6 space-y-8">
        ...
        <section>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <KpiCard label="إجمالي الأعضاء"    value={n(s?.total_members)}   icon={Users}      accent="bg-[#22334A]/10 text-[#22334A]"  />
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-black text-[#22334A]">الإجراءات السريعة</h2>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <Link to="/dashboard/hr/volunteer-requests" className="flex items-center gap-1 text-[11px] font-black text-[#2691C2] hover:underline">
                  عرض الكل <ArrowLeft size={12} />
                </Link>
                <h3 className="text-sm font-black text-[#22334A]">آخر طلبات التطوع</h3>
              </div>
                  ...
                  {data.recent_volunteer_requests.map((r) => (
                    <li key={r.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2.5">
                      ...
                      <div className="min-w-0 flex-1 text-right">
                        <p className="truncate text-sm font-black text-[#22334A]">{r.full_name}</p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <Link to="/dashboard/volunteer" className="flex items-center gap-1 text-[11px] font-black text-[#2691C2] hover:underline">
                  عرض الكل <ArrowLeft size={12} />
                </Link>
                <h3 className="text-sm font-black text-[#22334A]">مقبولون بانتظار التحويل</h3>
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <Link to="/dashboard/hr/instructors" className="flex items-center gap-1 text-[11px] font-black text-[#2691C2] hover:underline">
                  عرض الكل <ArrowLeft size={12} />
                </Link>
                <h3 className="text-sm font-black text-[#22334A]">آخر المدربين</h3>
              </div>
                ...
                <ul className="space-y-2">
                  {data.recent_instructors.map((i) => (
                    <li key={i.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2.5">
                      ...
                      <div className="min-w-0 flex-1 text-right">
                        <p className="truncate text-sm font-black text-[#22334A]">{i.name}</p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <Link to="/dashboard/hr/departments" className="flex items-center gap-1 text-[11px] font-black text-[#2691C2] hover:underline">
                  عرض الكل <ArrowLeft size={12} />
                </Link>
                <h3 className="text-sm font-black text-[#22334A]">توزيع الأعضاء بالإدارات</h3>
              </div>
                    ...
                    return (
                      <li key={d.name} className="grid gap-1 text-right">
                        <div className="flex items-center justify-between text-[12px]">
                          <span className="font-black text-[#22334A]">{n(d.members_count)}</span>
                          <span className="font-semibold text-slate-600">{d.name}</span>
                        </div>
                        <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
                          <div className="h-full rounded-full bg-[#2691C2] transition-all" style={{ width: `${pct}%` }} />
                        </div>
                      </li>
                    )
                  })}
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Link to="/dashboard/hr/onboarding" className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            ...
            <div className="text-right">
              <p className="text-sm font-black text-[#22334A]">التأهيل والانضمام</p>
          ...
          <Link to="/dashboard/hr/tasks" className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            ...
            <div className="text-right">
              <p className="text-sm font-black text-[#22334A]">مهام الموارد البشرية</p>
          ...
          <Link to="/dashboard/hr/documents" className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            ...
            <div className="text-right">
              <p className="text-sm font-black text-[#22334A]">ملفات الموارد البشرية</p>
