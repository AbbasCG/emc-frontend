  ...
  return (
    <section className={`rounded-2xl border border-slate-100 bg-white shadow-sm ${className}`}>
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
        ...
        {href && (
          <Link to={href} className="flex items-center gap-1 text-xs font-bold text-[#2691C2] hover:text-[#1A6CA8]">
            {hrefLabel ?? 'عرض الكل'}
            <ChevronLeft size={12} />
          </Link>
        )}
...
function Hero({
  ...
  return (
    ...
    >
      {/* Glow orbs */}
      <div className="pointer-events-none absolute right-0 top-0 h-48 w-48 -translate-y-1/3 translate-x-1/4 rounded-full bg-[#2691C2]/20 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 left-1/4 h-32 w-32 translate-y-1/2 rounded-full bg-[#2691C2]/10 blur-2xl" />
      ...
      <div className="relative flex flex-col justify-between gap-5 px-6 py-6 sm:flex-row sm:items-center sm:justify-between sm:gap-10 sm:px-8">
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#2691C2] to-[#1A6CA8] shadow-lg ring-2 ring-white/10">
            {avatarUrl
...
function KpiCard({
  ...
  return (
    ...
    >
      ...
      {urgent && value > 0 && (
        <span className="mr-auto shrink-0 rounded-full bg-[#EC943C] px-2 py-0.5 text-[10px] font-black text-white">{n(value)}</span>
      )}
    </motion.div>
  )
}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2 text-[11px] text-slate-400">
          {session.starts_at && <span><Clock size={10} className="mr-0.5 inline text-[#2691C2]" />{fmt24(session.starts_at)}{session.ends_at && `–${fmt24(session.ends_at)}`}</span>}
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        {isLive && session.meeting_link
          ...
          : !isDone && session.meeting_link
            ? <a href={session.meeting_link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 rounded-lg border border-[#2691C2]/20 bg-[#2691C2]/[0.07] px-2.5 py-1.5 text-[11px] font-black text-[#2691C2] hover:bg-[#2691C2]/[0.12]"><Video size={10} />فتح</a>
            : null}
...
function UpcomingRow({ session, i }: { session: EnrichedSession; i: number }) {
  ...
  return (
    ...
    >
      <div className="flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-xl bg-[#2691C2]/[0.08] text-center">
        <span className="text-[10px] font-black text-[#2691C2]">{dateLabel}</span>
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-[13px] font-black text-[#0B1E38]">
          {session.course_name ?? session.title ?? 'جلسة'}
      ...
      {session.meeting_link
        ? <a href={session.meeting_link} target="_blank" rel="noopener noreferrer" className="flex shrink-0 items-center gap-1 rounded-lg border border-[#2691C2]/20 bg-[#2691C2]/[0.07] px-3 py-1.5 text-[11px] font-black text-[#2691C2] hover:bg-[#2691C2]/[0.12]"><Video size={10} />فتح</a>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-slate-400">
          ...
          {sub.course_name && <><span>·</span><span className="truncate text-[#2691C2]">{sub.course_name}</span></>}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <Link
          ...
          className="flex items-center gap-1 rounded-lg bg-[#EC943C] px-3 py-1.5 text-[11px] font-black text-white hover:brightness-105"
        >
...
function ActiveCourseCard({ cp, i }: { cp: CoursePriority; i: number }) {
  ...
  return (
    ...
    >
      {/* Cover */}
      <div className="relative h-28 overflow-hidden bg-gradient-to-br from-[#0B1E38] to-[#2691C2]">
        {imageUrl
        ...
        {!hasLive && hasToday && (
          <span className="absolute left-3 top-3 rounded-full bg-[#2691C2] px-2 py-0.5 text-[9px] font-black text-white">اليوم</span>
        )}
        {pendingCount > 0 && (
          <span className="absolute right-3 top-3 rounded-full bg-[#EC943C] px-2 py-0.5 text-[9px] font-black text-white">{n(pendingCount)} تسليم</span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div className="flex items-center gap-3 text-[11px] font-semibold text-slate-500">
          <span className="flex items-center gap-1"><Users size={11} className="text-[#2691C2]" />{n(studentCount)}</span>
          {nextSession?.starts_at && (
            <span className="flex items-center gap-1 truncate"><Clock size={11} className="shrink-0 text-[#2691C2]" />{fmt24(nextSession.starts_at)}</span>
          )}
        </div>

        <Link
          to={`/dashboard/instructor/courses/${course.id}/students`}
          className="mt-auto flex items-center justify-center gap-1.5 rounded-xl bg-[#0B1E38]/[0.05] py-2 text-[11px] font-black text-[#0B1E38] transition hover:bg-[#2691C2]/[0.10] hover:text-[#2691C2]"
        >
        ...
        {data.map((d, i) => (
          <div key={i} className="flex flex-1 flex-col items-center gap-1.5">
            <motion.div
              ...
              className={`w-full rounded-t-lg ${d.highlight ? 'bg-[#2691C2]' : 'bg-slate-200'}`}
            />
            ...
            <>
              ...
              {submissionsPending > pendingSubmissions.length && (
                <div className="mt-3 border-t border-slate-100 pt-3 text-center">
                  <Link to="/dashboard/instructor/submissions" className="text-[12px] font-bold text-[#2691C2] hover:underline">
              ...
              {activeCourseCount > 6 && (
                <div className="mt-4 text-center">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 rounded-xl border border-[#2691C2]/20 bg-[#2691C2]/[0.06] px-5 py-2.5 text-[12px] font-black text-[#2691C2] transition hover:bg-[#2691C2]/[0.10]"
                  >
                    <BookOpen size={13} />
